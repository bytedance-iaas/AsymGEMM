...
def test_linear_attention_saved_tensor_offload_installs_in_place() -> None:
    module = SaveTensorModule()
    wrapper = install_linear_attention_saved_tensor_offload(module, min_bytes=0, require_grad=True)

    assert is_linear_attention_saved_tensor_offload_wrapper(module)
    assert install_linear_attention_saved_tensor_offload(module) is wrapper


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA saved tensor offload")
def test_linear_attention_saved_tensor_offload_preserves_backward_and_records_stats() -> None:
    torch.manual_seed(0)
    module = SaveTensorModule().cuda()
    install_linear_attention_saved_tensor_offload(module, min_bytes=0, require_grad=True)
    ...
    assert stats["linear_attention_saved_tensor_offload"] is True
...
def test_linear_attention_saved_tensor_offload_module_names() -> None:
    model = nn.Sequential(SaveTensorModule(), SaveTensorModule())
    install_linear_attention_saved_tensor_offload(model[0], min_bytes=0, require_grad=True)

    assert linear_attention_saved_tensor_offload_module_names(model) == ("0",)
