...
"""
Correctness + performance test for m_grouped_fp8_asym_gemm_sm80.
Reference: per-expert torch.matmul in float32, scaled by scale_a * scale_b.

Run:
    python tests/test_sm80_moe.py            # correctness + benchmark
    python tests/test_sm80_moe.py --perf     # benchmark only (faster)
"""
import gc
import itertools
import os
...
def test_fp8_moe_gemm(with_timing: bool = True):
    print("=" * 72)
    print("Correctness: m_grouped_fp8_asym_gemm_sm80 (SM89 native FP8 MMA)")
    print("=" * 72)
...
def benchmark_fp8_moe_gemm(num_warmup: int = 10, num_iters: int = 100):
    ...
    print()
    print("=" * 72)
    print("Performance: m_grouped_fp8_asym_gemm_sm80  "
