...
class SM100BF16EpStealAsymGemmRuntime final: public LaunchRuntime<SM100BF16EpStealAsymGemmRuntime> {
public:
    struct Args {
        int m, n, k, num_groups;
        const std::string& compiled_dims;
        ...
        // sEP steal (fix_gb200_ep.md S2b): union queue over [own | peer] sections;
        // stolen items read A from / store D to the FABRIC (pinned sysmem) via the
        // peer descriptors. ep_n_own is the section boundary in SEGMENT units.
        void* ep_queue;
        uint32_t ep_total_items;
        uint32_t ep_side;
        uint32_t ep_n_own;
        CUtensorMap tensor_map_a_peer;
        CUtensorMap tensor_map_cd_peer;
    };

    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#define ASYM_BF16_KERNEL_NAME sm100_bf16_asym_gemm_ep_steal_impl
#define ASYM_BF16_EP_QUEUED 1
#define ASYM_BF16_EP_STEAL 1
#define ASYM_BF16_KERNEL_EXTRA_ARGS , int* ep_queue, uint32_t ep_total_items, uint32_t ep_side, uint32_t ep_n_own, const __grid_constant__ cute::TmaDescriptor tensor_map_a_peer, const __grid_constant__ cute::TmaDescriptor tensor_map_cd_peer
#include <asym_gemm/impls/sm100_bf16_asym_gemm.cuh>

using namespace asym_gemm;

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&sm100_bf16_asym_gemm_ep_steal_impl<
        {}, {},
        {}, {}, {},
        {}, {}, {},
        {},
    ...
    static void launch_impl(const KernelHandle& kernel, const LaunchConfigHandle& config, Args args) {
        DG_CUDA_UNIFIED_CHECK(launch_kernel(kernel, config,
            ...
            args.tensor_map_a, args.tensor_map_b,
            args.tensor_map_cd,
            args.ep_queue, args.ep_total_items, args.ep_side, args.ep_n_own,
            args.tensor_map_a_peer, args.tensor_map_cd_peer));
    }
};
...
public:
    struct Args {
        ...
        CUtensorMap tensor_map_b_third;
        CUtensorMap tensor_map_cd_third;
...
static void __instantiate_kernel() {{
    ...
    static void launch_impl(const KernelHandle& kernel, const LaunchConfigHandle& config, Args args) {
        DG_CUDA_UNIFIED_CHECK(launch_kernel(kernel, config,
            ...
            args.tensor_map_b_third,
            args.tensor_map_cd_third));
    }
};
...
static int sm100_m_grouped_bf16_asym_gemm_contiguous_ep_steal(const torch::Tensor& a,
                                                 const torch::Tensor& b,
                                                 const torch::Tensor& d,
                                                 const torch::Tensor& a_peer,
                                                 const torch::Tensor& d_peer,
                                                 const torch::Tensor& offsets_t,
                                                 const torch::Tensor& experts_t,
                                                 const torch::Tensor& ep_queue_t,
                                                 const int& ep_side,
                                                 ...
                                                 const std::string& compiled_dims,
                                                 const int b_outer_stride = -1) {
    // sEP steal launch (fix_gb200_ep.md S2b): identical config to the queued launcher;
    // adds the FABRIC peer descriptors (A pack + D staging, both pinned sysmem — the B
    // path proves host TMA) and the section boundary in SEGMENT units.
    const int block_m = (major_b == cute::UMMA::Major::MN)
        ? get_env<int>("DG_BF16_TRANSPOSE_BLOCK_M", 64)
        : get_env<int>("DG_BF16_BLOCK_M", 64);
    const int block_n = (major_b == cute::UMMA::Major::MN)
    ...
    const auto& tensor_map_a = make_tma_a_desc(major_a, a, m, k,
                                               ...
                                               static_cast<int>(a.stride(get_non_contiguous_dim(major_a))), 1,
                                               config.smem_config.swizzle_a_mode);
    const auto& tensor_map_a_peer = make_tma_a_desc(major_a, a_peer, m_peer, k,
                                               SM100ArchSpec::get_ab_load_block_m(config.multicast_config, config.block_m),
                                               config.block_k,
                                               static_cast<int>(a_peer.stride(get_non_contiguous_dim(major_a))), 1,
                                               config.smem_config.swizzle_a_mode);
    const int outer_b = (b_outer_stride >= 0)
        ? b_outer_stride
        : static_cast<int>(b.stride(get_non_contiguous_dim(major_b)));
    ...
    const auto& tensor_map_cd = make_tma_cd_desc(d, m, n,
                                                 ...
                                                 static_cast<int>(d.stride(-2)), 1,
                                                 config.smem_config.swizzle_cd_mode);
    const auto& tensor_map_cd_peer = make_tma_cd_desc(d_peer, m_peer, n,
                                                 SM100ArchSpec::get_cd_store_block_m(config.block_m),
                                                 SM100ArchSpec::get_cd_store_block_n(config.block_n),
                                                 static_cast<int>(d_peer.stride(-2)), 1,
                                                 config.smem_config.swizzle_cd_mode);
    ...
    const SM100BF16EpStealAsymGemmRuntime::Args& args = {
        .m = m, .n = n, .k = aligned_k,
        .compiled_dims = compiled_dims,
        .gemm_config = config,
        .launch_args = LaunchArgs({num_n_blocks, grid_y_local}, config.thread_config.num_threads,
        ...
        .ep_total_items = ep_total_items,
        .ep_side = static_cast<uint32_t>(ep_side),
        .ep_n_own = static_cast<uint32_t>(ep_n_own),
        .tensor_map_a_peer = tensor_map_a_peer,
        .tensor_map_cd_peer = tensor_map_cd_peer
    };
    const auto& code = SM100BF16EpStealAsymGemmRuntime::generate(args);
    const auto& runtime = compiler->build("sm100_bf16_m_grouped_asym_gemm_contiguous_ep_steal", code);
    SM100BF16EpStealAsymGemmRuntime::launch(runtime, args);
    // the owner-side gather needs the item granularity; only the launcher knows block_n
    return num_n_blocks;
}
...
static void sm100_m_grouped_bf16_cpu_left_asym_gemm_contiguous(const torch::Tensor& a,
    ...
    const SM100BF16CpuLeftAsymGemmRuntime::Args& args = {
        ...
        .tensor_map_b_third = tensor_map_b,
        .tensor_map_cd_third = tensor_map_cd,
...
static void sm100_m_grouped_bf16_cpu_left_pair_asym_gemm_contiguous(const torch::Tensor& a,
    ...
    const SM100BF16CpuLeftAsymGemmRuntime::Args& args = {
        ...
        .tensor_map_b_third = tensor_map_b_up,
        .tensor_map_cd_third = tensor_map_d_up,
        .compact_m_block_grid = compact_m_block_grid,
        .num_outputs = 2
    };
    const auto& code = SM100BF16CpuLeftAsymGemmRuntime::generate(args);
...
// N1 (2026-07-29): qkv shared-stream triple — one host-activation stream
// serves THREE adapter weight/output sets through the same fetch-once slot.
static void sm100_m_grouped_bf16_cpu_left_triple_asym_gemm_contiguous(const torch::Tensor& a,
    ...
    const auto& config = get_manual_config_asym<SM100ArchSpec>(
        GemmType::MGroupedContiguous, KernelType::KernelNoSF,
        m, n, k, 1, major_a, major_b,
        torch::kBFloat16, d0.scalar_type(), false,
        device_runtime->get_num_sms(),
        block_m, block_n, block_k);
    DG_HOST_ASSERT(config.multicast_config.num_multicast == 1);
    ...
    auto make_b = [&](const torch::Tensor& b) {
        return make_tma_b_desc(major_b, b, n, k,
                               SM100ArchSpec::get_ab_load_block_n(config.multicast_config, config.block_n),
                               config.block_k, outer_b, num_groups,
                               config.smem_config.swizzle_b_mode);
    };
    auto make_d = [&](const torch::Tensor& d) {
        return make_tma_cd_desc(d, m, n,
                                SM100ArchSpec::get_cd_store_block_m(config.block_m),
                                SM100ArchSpec::get_cd_store_block_n(config.block_n),
                                static_cast<int>(d.stride(-2)), 1,
                                config.smem_config.swizzle_cd_mode);
    };
    const auto& tb0 = make_b(b0); const auto& tb1 = make_b(b1); const auto& tb2 = make_b(b2);
    const auto& td0 = make_d(d0); const auto& td1 = make_d(d1); const auto& td2 = make_d(d2);
    ...
    const SM100BF16CpuLeftAsymGemmRuntime::Args& args = {
        ...
        .tensor_map_b = tb0,
        .tensor_map_cd = td0,
        .tensor_map_b_pair = tb1,
        .tensor_map_cd_pair = td1,
        .tensor_map_b_third = tb2,
        .tensor_map_cd_third = td2,
        .compact_m_block_grid = compact_m_block_grid,
        .num_outputs = 3
    };
    const auto& code = SM100BF16CpuLeftAsymGemmRuntime::generate(args);
    const auto& runtime = compiler->build("sm100_bf16_m_grouped_cpu_left_triple_asym_gemm_contiguous", code);
    SM100BF16CpuLeftAsymGemmRuntime::launch(runtime, args);
}
