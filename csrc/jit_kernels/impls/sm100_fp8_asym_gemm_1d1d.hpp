...
// ============================================================================
// Masked GEMM Implementation for FP8
// ============================================================================
class SM100FP8AsymGemmMaskedRuntime final: public LaunchRuntime<SM100FP8AsymGemmMaskedRuntime> {
public:
    struct Args {
        int m, n, k, num_groups;
        const std::string& compiled_dims;
...
static void sm100_m_grouped_fp8_asym_gemm_masked_1d1d(const torch::Tensor& a, const torch::Tensor& sfa,
    ...
    const auto& code = SM100FP8AsymGemmMaskedRuntime::generate(args);
    const auto& runtime = compiler->build("sm100_m_grouped_fp8_asym_gemm_masked_1d1d", code);
    SM100FP8AsymGemmMaskedRuntime::launch(runtime, args);
}
