...
static torch::Tensor get_mn_major_tma_aligned_packed_byte_sf_tensor(const torch::Tensor& sf) {
    ...
    // Reinterpret FP8 payload bytes directly (no numeric conversion).
...
static torch::Tensor transform_sf_into_required_layout(const torch::Tensor& sf,
    ...
    // Keep FP32 scales and only convert to MN-major, TMA-aligned layout.
    ...
    // Keep original E4M3 payload bytes and only repack to MN-major, TMA-aligned
    // uint32 layout expected by block-scaled NVFP4 MMA.
