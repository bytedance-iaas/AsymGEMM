...
/* True when the runtime-selected INT8 row-major backend (AMX or
 * AVX-512-VNNI, dispatch/int8_rm_backend) can handle this descriptor —
 * i.e. B is row-major int8 + per-channel scales, K is K_STEP-aligned for
 * the chosen backend, A is tightly packed BF16, and B's logical N is
 * N_STEP-aligned (so strided loads never read past the caller's
 * allocation).
 *
 * The unified-MoE runtime always pads N to 128 (GPU constraint), which is
 * a multiple of both backends' N_STEP (AMX 32, VNNI 16), so this predicate
 * covers it. Unaligned-N callers still hit the packed-on-touch AMX path
 * below. */
bool int8_rm_eligible(const cg_gemm_desc_t* d,
                      const cpu_gemm::Int8RmBackend& be) {
  if (!be.ok) return false;
  if (!is_bf16_int8_f32_nt(d)) return false;
  if (d->k % be.k_step != 0) return false;
  if (d->n % be.n_step != 0) return false;
  if (d->lda != d->k)          return false;   /* A still tight row-major BF16 */
  if (d->ldb <  d->k)          return false;   /* B row-major, stride >= k */
  return true;
}

/* Stride-aware row-major B path, backend-agnostic. No per-call B pack — B
 * is read straight from caller memory (AMX: strided _tile_loadd; VNNI:
 * strided vector loads). A is packed once into the backend's shape; C
 * lands as C^T and the unpack transposes during scale apply. See
 * Stride.md §3 for the derivation; both kernels self-configure per thread
 * inside run(). */
cg_status_t run_int8_rm(cg_runtime_t* rt, const cg_gemm_desc_t* d,
                        const cpu_gemm::Int8RmBackend& be) {
  int m = (int)d->m, n = (int)d->n, k = (int)d->k;

  auto s = be.scratch_sizes(m, n, k);
  void* base = rt->scratch->reserve(s.total());
  auto* p = reinterpret_cast<unsigned char*>(base);
  void* sA = p;            p += s.bytes_a;
  void* sC = p;
  ...
  const float* a_scales = reinterpret_cast<const float*>(
      reinterpret_cast<unsigned char*>(sA) + s.a_scales_off);

  int n_pad    = (n + be.n_step - 1) / be.n_step * be.n_step;
  int n_blocks = (n_pad + be.n_block - 1) / be.n_block;
  int nth = rt->threads;
  if (nth > n_blocks) nth = n_blocks;
  if (nth <= 0) nth = 1;
  ...
  if (nth == 1) {
    be.pack_a(m, k, a_rm, sA);
    be.run(m, n, k, b_rm, d->ldb, sA, sC, 0, 1);
    be.unpack_transposed(m, n, a_scales, b_scales_in, sC,
                         d->alpha, d->beta, c_rm, d->ldc, 0, 1);
  } else {
    /* A pack is cheap and single-threaded (m is small); run-and-unpack
     * fan out across threads along whole N_BLOCK stripes. */
    rt->pool->parallel_for(nth, [&](int ith) {
      if (ith == 0) be.pack_a(m, k, a_rm, sA);
    });
    rt->pool->parallel_for(nth, [&](int ith) {
      be.run(m, n, k, b_rm, d->ldb, sA, sC, ith, nth);
    });
    rt->pool->parallel_for(nth, [&](int ith) {
      be.unpack_transposed(m, n, a_scales, b_scales_in, sC,
                           d->alpha, d->beta, c_rm, d->ldc, ith, nth);
...
cg_status_t run_cg_gemm(cg_runtime_t* rt, const cg_gemm_desc_t* d) {
   ...
   * scratch. The backend (AMX INT8, else AVX-512-VNNI) is selected once
   * per process by select_int8_rm_backend(); ASYM_GEMM_FORCE_BACKEND
   * overrides for testing. The packed-on-touch fallback remains for
   * shapes that don't satisfy the stride-aware predicate (e.g.
   * n % N_STEP != 0). */
  if (rt && rt->pool) {
    const auto& be = cpu_gemm::select_int8_rm_backend();
    if (int8_rm_eligible(d, be)) {
