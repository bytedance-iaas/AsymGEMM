...
__global__ void cpuAwareTilingWithoutBias(Params params) {
    ...
    for (int e = 0; e < params.list_size; e++) {
        int expert_id = params.expert_list[e];
        int len = params.index_list[e] - len_start;

        Tensor gW = local_tile(mW(expert_id, _, _), Shape<Int<kBlockN>, Int<kBlockK>>{}, make_coord(x_idx, _));
        ...
        Tensor gO = local_tile(mO, Shape<Int<kBlockM>, Int<kBlockN>>{}, make_coord(_, x_idx));
        ...
        //     print("\n params.index_list[e]: %d ", params.index_list[e]);
        //     print("\n len_start: %d ", len_start);
        //     print("\n len: %d ", len);
        //     print("\n m_max: %d ", m_max);
        //     printf("A_ptr params.x_ptr = %p\n", (void*)params.x_ptr);
        //     printf("A_ptr params.x_ptr = %p\n", (void*)params.x_ptr + len_start * params.K);
...
void launch_MoECompute(cutlass::half_t* A, cutlass::half_t* B, cutlass::half_t* C, int N, int K, int expert_size, int list_size, int* expert_list, int* index_list, cudaStream_t stream) {
    // Initialize Params
    Params params;
    params.x_ptr = A;
    params.w_ptr = B;
    ...
    // kHeadDim_, kBlockM_, kBlockN_
    // cpuAwareTiling<Kernel_traits><<<grid, block, smem_size>>>(params);
    if (params.N == 4096)
    {
        dim3 grid(128);
        using KT = Flash_fwd_kernel_traits<32, 128, 32, 4, false, false, cutlass::half_t>;
        asymComputeWithoutBias<KT>(params, grid, stream);
    }
    else if (params.N == 4096 * 4)
    {
        dim3 grid(128);
        using KT = Flash_fwd_kernel_traits<128, 128, 128, 4, false, false, cutlass::half_t>;
        asymComputeWithoutBias<KT>(params, grid, stream);
    }
    else
    {
    ...
    // asymCompute<<<grid, block, smem_size>>>(params);
}
