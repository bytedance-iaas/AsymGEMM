...
mean of middle 2; **same-day adjacent-pair A/Bs only** (measured day drift: ±0.1 s @32k,
±10 s @128k); flags verified via `/proc/<gpu-pid>/environ` + ENGAGED markers; leaves
...
4. **Honest negatives (measured, cited as findings):** batch scaling ≠ throughput at
   32k (GPU already compute-bound; b24 −12.4% as the policy sheds wins); dense-model
