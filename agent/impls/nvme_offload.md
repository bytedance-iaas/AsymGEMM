...
- NOT reusable: `AsyncPartitionedParameterSwapper` (needs `ds_id`/`ds_tensor`, built only in `zero.Init` — `partitioned_param_swapper.py:37,76-161`, `partition_parameters.py:1081-1095`); `SwapBufferManager` (`dist.get_rank()` — `swap_tensor/utils.py:181,:195`); GDS (no hardware path).
