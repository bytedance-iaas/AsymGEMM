...
- **H3b (new code)**: `ASYMM_QWEN3_MOE_FG_FWD_BLOCK_EXPERTS` — row-blocked
  ...
  - **SUPERSEDED 2026-07-15 (knob retired, technique kept)**: the SFT memory/latency
    line landed the same row-blocking independently as `_fg_elementwise_blocks`, which
    derives the expert-count from the `ASYMM_FG_ELEMENTWISE_CHUNK_MB` byte budget and
    then calls the same `_expert_blocks` this knob called — so H3b's win is realized by
    default, without the knob. Their default (1024 MB) was probe-swept for reserved
    (fragmentation gap 21→5 GiB @120k; 512 AND 2048 both worse), i.e. finer blocking is
    non-monotonic — worth remembering before hand-picking a finer block count.
    To re-tune the block granularity, move `ASYMM_FG_ELEMENTWISE_CHUNK_MB`, not an
    expert count. Their path is gated on `da_gpu and lora_a_fwd_gpu and not
    keep_acts_hbm`, so it does NOT engage when `DOWN_SCATTER_BLOCK_EXPERTS>0` (which
    forces da_gpu off) — the one coverage gap vs H3b, which ran with dscatter.
  - **Still open (deferred at merge)**: the nograd forward keeps a full-R `act`
    [R, I] bf16 (~1.4 GiB @30B-a3b/120k/b8) that H3b's blocked down+scatter avoided.
    Live under GC (the outer forward is no-grad). Re-port if the fwd peak needs it.
