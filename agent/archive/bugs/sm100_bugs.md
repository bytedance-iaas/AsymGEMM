# AsymGEMM SM100 LoRA-SFT — Relevant Bugs / Findings

> Generated 2026-05-31. **Filtered extract** of the full review (`agent/bugs/bugs.md`), scoped to the **SM100 (Blackwell GB200) BF16 LoRA-SFT path**: the `sm100_bf16_asym_gemm` forward + `transpose_b` dx kernels, the shared scheduler/TMA/JIT layer they use, the SFT Python layer (frozen_linear / lora / moe / qwen3_moe), and the LlamaFactory integration that wires it.

> **Scope rules.** The LoRA-SFT path is **bf16-only** (`asym_precision: Literal["bf16"]`). Therefore: **(a) EXCLUDED** — the SM80/SM89 (Ada/PCIe `cp.async`) MoE kernels (all of K3), which are a different architecture and not used on SM100; **(b) Appendix A** — the SM100 FP8/FP4 kernels and the quantized frozen-weight path, which exist on SM100 but are NOT exercised by the current bf16-only SFT (kept for when fp8/fp4 SFT is enabled); **(c) Main body** — everything that runs during a bf16 LoRA-SFT step on SM100. No source files were modified.

> **Tally (SM100 bf16 path):** 42 confirmed on-path findings + 1 uncertain · Appendix A: 5 SM100 fp8/fp4 (not on bf16 path) · Excluded: 6 SM80/SM89 findings.


## SM100 bf16-path fix priority (highest first)

1. 🔴 **FP32 K-tile partial-sum accumulation** — the SM100 BF16 kernel REDUCE_ADDs partials in bf16 in HBM for every K>BLOCK_K GEMM (forward + dx); precision loss grows with K-tile count (K1.1).
2. 🟡 **Add the dense `k%64` dx alignment guard** — `transpose_b` dx contraction is `out_features`; the dense path lacks the guard the grouped path has → silently-wrong gradient when `out_features` is 8- but not 64-aligned (K5.1).
3. 🔴 **Wire `compare_lf_smoke_losses.py` + add a Blackwell GPU CI job** — all SM100 kernel parity tests are GPU-skip-gated, so kernel regressions are invisible in CI (I3.1, I3.2).
4. 🔴 **Default transpose `block_m=128` + per-shape block-tile autotuning** — the repo's own sweep shows block_m=128 is ~1.6× faster for the dx path; biggest measured SM100 latency win (K6.1, L5.4/L5.3).
5. 🟡 **Add `.num_groups` to the SM100 BF16 Args** (kNumGroups baked as 0) (K6.4); **add offsets/experts validation completeness** (K6.5).
6. **Cut per-step host stalls:** cache device-resident quantized weight if fp8/fp4 is used (L1.2); remove route-validation `.item()` D2H syncs on the hot Qwen3 path (L3, L4.1); avoid the grouped `offsets.tolist()` sync + pad/unpad copies (L1.3).
7. 🟡 **Launcher:** auto-export `USE_ASYM_GEMM` from the yaml flag, or add a parser guard forbidding DDP under asym (I2.1).

---

## Main body — bugs on the SM100 BF16 LoRA-SFT path


### [K1] BF16 asym GEMM device kernel — SM100
...
> **[SM100-verified]** The cited line is in the `sm90` sibling, but the identical defect is in the **SM100** kernel: `sm100_bf16_asym_gemm.cuh:881-885` casts the FP32 TMEM accumulator to bf16 via `cast_into_bf16_and_pack`, then `:1001-1003` issues `SM90_TMA_REDUCE_ADD_2D` for every `block_k_iter > 0` (line 984 does `STORE_2D` only for k=0). So multi-K-tile forward (block_k=512) and dx (block_k=256) accumulate partials in bf16 in HBM. Confirmed by direct read.
...
### [K4] asymScheduler / schedulers / TMA + common utils (SM100 path)
...
### [K5] Transpose variant (transpose_b) — LoRA dx path on SM100 BF16
...
#### K5.2. ⚪ LOW · design · einsum (bmk_bnk_mn / bhr_hdr_bhd), smxx_clean_logits, and k_grouped_fp8 are inherited DeepGEMM code not used by the LoRA dx path
...
#### K5.3. ⚪ LOW · correctness · Masked BF16 wrapper has no transpose_b parameter; masked-layout dx is unsupported (acceptable but undocumented)
...
### [K6] Host launchers / tiling heuristics / JIT codegen (SM100 BF16)
...
#### K6.2. ⚪ LOW · correctness · SM100 BF16 asym launchers omit .num_groups initializer; kNumGroups is baked as 0 into the JIT template
...
#### K6.3. ⚪ LOW · correctness · int-list_size contiguous validation under-checks offsets.numel() vs the pairs layout the scheduler indexes
...
#### K6.4. ⚪ LOW · design · CPU-residency of B is assumed by the asym design but never enforced/asserted on the SM90/SM100 BF16/FP8 paths
...
### [L4] Qwen3 expert wrapping + host-weight offload (qwen3_moe.py)
...
### [L5] LoRA-SFT efficiency from profiling artifacts (SM100 GB200)
...
## ⚠️ Uncertain — needs a Blackwell GPU run to settle
...
## Appendix A — SM100 FP8/FP4 kernels (present, but NOT exercised by the current bf16-only LoRA-SFT path)

_These are real SM100 findings; they become live only when fp8/fp4 SFT is enabled. Kept here so they aren't lost._

#### A.1. ⚪ LOW · improvement · FP4 NVFP4 path omits the per-tensor second-level FP32 scale, capping dynamic range
...
#### A.2. ⚪ LOW · design · Dead per-lane B-descriptor staging in MMA warp (only stage 0 / index 0 ever used for B)
...
#### A.3. 🟡 MEDIUM · efficiency · Quantized (fp8/fp4) dx materializes and re-quantizes a transposed weight copy, doubling pinned-CPU memory
...
#### A.4. 🟡 MEDIUM · efficiency · FP4 contiguous launcher emits 19 unconditional fprintf(stderr) on every launch
...
#### A.5. 🟡 MEDIUM · correctness · FP4 contiguous dispatcher performs no validation of offsets/experts
...
## Appendix B — EXCLUDED (SM80/SM89 Ada/PCIe `cp.async` path — different architecture, not SM100)

_Listed by title only; full detail is in `bugs.md`. Note the well-known FP8 `__syncthreads()` race is **SM89-only**, so it is NOT an SM100 bug._

- 🔴 HIGH · correctness · Missing __syncthreads() before sX LDSM in FP8 full-tile k>0 path (cross-thread smem race) — `asym_gemm/include/asym_gemm/impls/sm80_moe_gemm.cuh:714-721,750-753`
- 🔴 HIGH · correctness · Same missing sX-LDSM barrier in the masked FP8 kernel full-tile k>0 path — `asym_gemm/include/asym_gemm/impls/sm80_moe_gemm.cuh:1048-1055,1083-1086`
- 🔴 HIGH · efficiency · BF16 sm80 kernel re-fetches the entire W tile per M-tile (abandons K-outer weight reuse) — `asym_gemm/include/asym_gemm/impls/sm80_moe_gemm.cuh:307-359`
- 🟡 MEDIUM · efficiency · Single-buffered cp.async fully exposes PCIe/HBM fetch latency (no pipelining) — `asym_gemm/include/asym_gemm/impls/sm80_moe_gemm.cuh:330-346,641-668,696-717`
- ⚪ LOW · efficiency · Per-M-tile global read-modify-write of BF16 partial sums adds O(M_tiles*K_tiles) HBM O traffic — `asym_gemm/include/asym_gemm/impls/sm80_moe_gemm.cuh:693-768`
- ⚪ LOW · correctness · FP8 path passes b.data_ptr() raw for CPU-pinned weights; relies on UVA device-accessibility (unverified at API boundary) — `csrc/jit_kernels/impls/sm80_moe_gemm.hpp:153,155 and csrc/apis/gemm.hpp:761-763`

---

## Appendix C — Refuted false-alarms relevant to the SM100/SFT path (NOT bugs)

_Recorded with the reason they are not bugs, so they aren't re-investigated._


### C1. [K4] ~~asymScheduler has no guard for the -1 sentinel expert; correctness depends entirely on host launching gridDim.y = num_active (list_size-1)~~ (claimed medium/correctness)

- **Location cited:** `asym_gemm/include/asym_gemm/common/asymScheduler.cuh:95-107`
- **Claim:** In the contiguous/MGrouped path the constructor unconditionally does expert_id = experts[blockIdx.y]; n_start = expert_id*blocks_perExpert; m_start=ceil_div(offsets[blockIdx.y*2],BLOCK_M); m_end=ceil_div(offsets[blockIdx.y*2+1],BLOCK_M). There is NO check for the -1 sentinel. The metadata convention (see _dense_offsets / _group_metadata_tensors) gives experts.numel()=num_active+1 (last = -1) but offsets only holds 2*num_active pair entries. If the compiled binding ever launches gridDim.y = list_size (sentinel included) instead of list_size-1, blockIdx.y=num_active reads experts[num_active]=-1 -> expert_id=0xFFFFFFFF and offsets[2*num_active], offsets[2*num_active+1] which are OUT OF BOUNDS; n_idx = blockIdx.x*BLOCK_N + shape_n*0xFFFFFFFF overflows and the CTA would issue OOB TMA loads/stores. The kernel-visible code cannot self-protect against this; it is silently delegated to the host grid computation.
...
### C2. [L1] ~~Fallback statistics (record_fallback / fallback_reasons) are dead code; fallback bookkeeping never populates~~ (claimed medium/correctness)

- **Location cited:** `asym_gemm/training/frozen_linear.py:906,984,56`
- **Claim:** VALID_BACKENDS is only ('asym','torch'). In _dispatch_nt/_dispatch_grouped_nt the `record_fallback` calls (906, 984) are reachable only inside the `backend != 'torch'` block, i.e. only when backend=='asym'. But on every code path that reaches record_fallback in that block, the very next statement raises (907-916 / 985-994) because `backend == 'asym'`. On a direct RuntimeError the code re-raises at 904/982 before recording. Consequently fallback_reasons is always empty, and torch_forward_calls/torch_dx_calls only increment for a deliberate full-torch run, never as an asym->torch fallback. The AsymExecutionStats fallback machinery therefore reports nothing meaningful.
...
### C3. [L5] ~~Operator benchmark methodology gaps: no L2 flush, CUDA-graph may freeze the per-call fetch path, and FP8 baseline is asymmetric — can distort absolute asym-vs-torch numbers~~ (claimed medium/design)
...
- **Claim:** Neither measure() nor measure_cuda_graph() flushes the L2 cache between iterations, so the small repeated operands stay cache-resident and timings are optimistic for both backends — but more so for the small torch baseline weight, which biases the asym/torch ratio. CUDA-graph capture of the asym path can also bake in a single static fetch/quantization, potentially under-counting the real per-call CPU-fetch cost the design pays in training. In profile_transpose.py the FP8 comparison times torch in BF16 vs asym in FP8 and re-quantizes activations inside the timed asym call but not for torch.
...
### C4. [L5] ~~E2E profiling uses synthetic 'learned'/'balanced' route patterns and 1-layer (--profile-layers 1/4); real-workload generality of the asym-vs-torch ratio is unconfirmed~~ (claimed low/design)
...
### C6. [I1] ~~Asym MoE path is incompatible with the installed (ModuleList) Qwen3 MoE format and hard-fails under strict~~ (claimed medium/correctness)

- **Location cited:** `asym_gemm/training/qwen3_moe.py:50-62, asym_gemm/integrations/lf.py:188-189`
- **Claim:** is_qwen3_experts requires packed 3D gate_up_proj/down_proj Parameters plus num_experts/hidden_dim/intermediate_dim attrs. The transformers version present in this environment uses the legacy nn.ModuleList of per-expert Qwen3MoeMLP (gate_proj/up_proj/down_proj), for which is_qwen3_experts returns False. With wrap_experts True (target hits experts/all) and strict=True, no experts are found and lf.py:189 raises 'found no Qwen3 packed expert modules', so the asym MoE path cannot run on this transformers build. This is a version-coupling fragility, not a logic bug, but it silently depends on a transformers version exposing the packed format.
...
### C8. [I2] ~~asym arg-validation block is monolithic and not factored like KT's apply_kt_config, increasing drift risk~~ (claimed low/design)

- **Location cited:** `src/llamafactory/hparams/parser.py:337`
- **Claim:** All asym validation lives as an inline ~40-line block in get_train_args, whereas KT factors its config handling into KTransformersArguments.apply_kt_config / get_kt_config_dict (model_args.py:494-538) invoked at parser.py:564. The asym checks are correct and complete for the listed unsupported features, but co-locating them with the dataclass (as KT does) would keep the two backends symmetric and reduce the chance that a future edit updates one guard set and not the other. Pure maintainability note; no behavioral issue.
