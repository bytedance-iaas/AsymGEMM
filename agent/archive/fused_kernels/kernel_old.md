# SM100 BF16 Qwen3 Gate/Up Recompute Backward: Windowed W-Cache Plan

This is the implementation target for the V1.5 selected-recompute gate/up
...
The corrected design is not the old tiny `P`-panel cache. The first
implementation must be `cache_first_window`:

```text
CPU W_gate_up window -> HBM w_cache
HBM w_cache -> selected gate/up recompute + activation backward
...
```text
Native kernel does not own dropout RNG or dropout masks.
If lora_dropout == 0.0:
  Python may recompute gate/up low-rank S before the native call.
If lora_dropout > 0.0:
  Python must pass saved forward S_gate/S_up for selected rows.
Backward must not rerun dropout.
```

Do not expand V1 into full MLP backward fusion until this narrower selected
gate/up-base path passes correctness and timing gates.
...
Use from Megakernels:
  controller/loader/consumer/storer separation only after phase-split V1A proves
...
This means V1A must remain a simple phase-split `cache_first_window` until Stage
...
Before this native call, Python or the existing backward must compute:

```text
dact_base = dY W_down
dact_lora = d LoRA_down / d act
dact = dact_base + dact_lora
```

For every selected row `m`, expert `e`, and intermediate index `i`:
...
silu_gate = silu(gate)
dgate = dact_sel[m, i] * up * silu_grad(gate)
dup   = dact_sel[m, i] * silu_gate
...
BN:
  ...
  BN = 2P.
  Default BN=64.
  Not an independent API or CLI knob in V1.
...
Optional research path after V1A and V1B:
...
full-step latency is not better than V1A and V1B
```

Do not implement V1B or V1C before V1A is correct and profiled.

## Native API

Add one pybind-visible function:
...
grad_x_base_sel, grad_gate_sel, grad_up_sel, stats = asym_gemm.qwen3_gate_up_recompute_bwd_sm100_bf16_windowed(
    ...
    dact_sel,              # cuda bf16 [M_selected, I], contiguous
...
csrc/python_api.cpp
  ...
  register qwen3_gate_up_recompute_bwd_sm100_bf16_windowed

asym_gemm/__init__.py
  add qwen3_gate_up_recompute_bwd_sm100_bf16_windowed to _maybe_import_from_C
...
asym_gemm/training/qwen3_moe.py
  call the native function from _ThresholdedQwen3ExpertFunction.backward for
  selected recompute rows only
...
```text
csrc/apis/qwen3_moe.hpp
  recompute_activation direct/test entry

csrc/jit_kernels/impls/sm100_qwen3_gate_up_windowed_bwd.hpp
  recompute_grad_window launcher
  activation backward launcher

asym_gemm/include/asym_gemm/impls/sm100_qwen3_gate_up_windowed_bwd.cuh
  sm100_qwen3_gate_up_activation_grad_bf16
  optional fused recompute+activation epilogue only after simple path passes

csrc/apis/gemm.hpp
  reuse or expose grouped BF16 GEMM helper reading CUDA w_cache

scripts/lf/validate_qwen3_gate_up_windowed_bwd.py
  op=recompute_activation
...
```text
csrc/apis/qwen3_moe.hpp
  qwen3_gate_up_recompute_bwd_sm100_bf16_windowed host API

csrc/python_api.cpp
  public pybind qwen3_gate_up_recompute_bwd_sm100_bf16_windowed

asym_gemm/__init__.py
  expose qwen3_gate_up_recompute_bwd_sm100_bf16_windowed through _maybe_import_from_C
...
tests/training/test_qwen3_gate_up_windowed_bwd.py
  full native direct torch-reference tests
```

### Stage 5: Python Integration Dropout 0.00
...
scripts/lf/validate_qwen3_gate_up_windowed_bwd.py
  stage5_python_integration_drop000
...
```text
asym_gemm/training/qwen3_moe.py
  saved S_gate/S_up selection for native call
  dropout ownership counters

scripts/lf/validate_qwen3_gate_up_windowed_bwd.py
  stage6_python_integration_drop010

tests/training/test_lf_qwen3_asym_backend.py or tests/training/test_qwen3_gate_up_windowed_bwd.py
  dropout saved-S tests
  no backward dropout rerun tests
```

Do not pass dropout masks or RNG state into native C++ in Stage 6.
...
No new kernel features should be introduced in Stage 10. Kernel iteration belongs
...
No native ownership of dropout masks.
```
...
model is updated. The V1 cache budget assumes chunk-local [M_work, Kdx].
```
...
use pair offsets plus a sentinel expert list. V1 native code must own this
...
qwen3_gate_up_recompute_bwd_sm100_bf16_windowed(...)
  ...
  for iwin0 in 0..I step W:
    for expert_chunk in selected experts step G_work:
      ...
      Phase A1: recompute_grad_window
      Phase B:  dx_window

  cast/store grad_x_base_sel
  return grad_x_base_sel, grad_gate_sel, grad_up_sel, stats
```
...
Captured launch overhead should be a few milliseconds. Uncaptured Python launch
overhead can be larger; the validation must measure it.
...
metadata_ms should be < 1 percent of direct native time for qwen_shape_routed
...
### Phase A1: Recompute And Activation Backward

Implementation can use existing SM100 BF16 grouped GEMM plus a custom activation
epilogue kernel first. A later persistent kernel may fuse them.
...
for e_rel, e in expert_chunk:
  ...
  for n0 in 0..Kdx step BN:
    pair_acc = X_sel[rows_e_global, :] @ w_cache[e_rel, n0:n0+BN, :].T
```
...
```text
rows_e_local = row_to_chunk_local[rows_e_global]
for local_row, row in zip(rows_e_local, rows_e_global):
  for local column in 0..P:
    i = iwin0 + q_panel * P + local column
    if i >= I:
      grad_pair_window[local_row, n0 + local] = 0
      grad_pair_window[local_row, n0 + P + local] = 0
      continue

    gate = pair_acc_gate + lora_scale * dot(gate_low_rank_sel[row], gate_lora_B[e, i])
    up   = pair_acc_up   + lora_scale * dot(up_low_rank_sel[row],   up_lora_B[e, i])
    dact = dact_sel[row, i]

    dgate = dact * up * silu_grad(gate)
    dup   = dact * silu(gate)

    grad_pair_window[local_row, n0 + local] = dgate
    grad_pair_window[local_row, n0 + P + local] = dup
    grad_gate_sel[row, i] = dgate
    grad_up_sel[row, i] = dup
```
...
```text
recompute_grad_window_ms reported
recompute_reads_w_cache_bytes > 0 for cache_first_window
local_memory_load/store bytes reported
pair_acc_global_bytes reported
tensor-core utilization reported in NCU for GEMM kernels
activation kernel register count and local memory bytes reported
```
...
an acceptable substitute must be an in-repo native C++/CUDA helper called from
...
ASYM_QWEN3_GATE_UP_WINDOWED_BWD=1:
  use qwen3_gate_up_recompute_bwd_sm100_bf16_windowed for selected recompute
...
selected-recompute backward. V1 native C++ does not accept FP16/FP32 LoRA low
rank or LoRA-B tensors.

Backward ownership:

```text
1. existing down path computes dact
2. compact selected rows into x_sel and dact_sel
3. provide gate_low_rank_sel/up_low_rank_sel:
     dropout 0.0: recompute S in Python or use saved S
     dropout >0.0: use saved forward S
4. call qwen3_gate_up_recompute_bwd_sm100_bf16_windowed with
     layer.gate_up_base.host_weight.weight
5. scatter grad_x_base_sel to selected rows in grad_packed
6. scatter grad_gate_sel/grad_up_sel for existing LoRA backward
7. run existing nonselected gate/up base dX only for nonselected rows
8. run existing gate/up LoRA backward for all rows needing LoRA grads/dX
```
...
for fill_w_cache, recompute, activation backward, and dX kernels with all
...
Required before LF E2E:
  ...
  for qwen_shape_smallM and at least one large/routed case, preferably
  qwen_shape_routed.
...
There is no `--bn` argument in V1. `BN` is always derived as `2 * p`.
...
recompute_activation:
  run metadata, fill_w_cache, recompute, and activation backward

dx_window:
  run metadata, fill_w_cache, recompute, activation backward, and dX
...
cache_first_profile:
  run Stage 7 aggregate NSYS/NCU profile validation from direct op artifacts
...
```json
{
  "stage": "stage4_native_direct_e2e",
  "op": "native_e2e",
  "passed": true,
  "device": {"name": "...", "sm": 100},
...
ncu_recompute_activation:
  validate_qwen3_gate_up_windowed_bwd.py --op recompute_activation --profile-mode ncu
...
<OUTPUT_ROOT>/native<NATIVE_ENABLED>__<mode>__p<P>_q<Q>_bm<BM>_bk<BK>_gw<G_WORK>/
  ...
  ncu_recompute_activation/validation.json
...
native_total_ms, fill_w_cache_ms, recompute_grad_window_ms, dx_window_ms
current_selected_region_ms
native_selected_region_ms
selected_region_speedup
...
recompute:
  op=recompute_activation, grouped BF16 GEMM kernel reading w_cache

activation_backward:
  op=recompute_activation, activation grad kernel
...
fill_w_cache:
  ...
  Local memory load/store bytes are near zero.
  Bandwidth is high enough that fill_w_cache is not dominated by scalar address
  overhead.

recompute:
  DRAM/L2 read bytes are reported and compared with R_M tile model.
  Tensor-pipe utilization is nonzero and materially used.
  Local memory bytes are small.
  Long-scoreboard stalls explain memory-bound behavior if tensor utilization is
  low.

activation_backward:
  Local memory bytes are near zero.
  Register count does not force very low occupancy.
  Math latency is not dominant compared with recompute/dX GEMM.

dx_window:
  Tensor-pipe utilization is nonzero and materially used.
  DRAM/L2 read bytes include w_cache reads and are compared with R_M tile model.
  dx_acc read/write bytes are reported.
  repeated dx_acc traffic is not exploding with small Q.
```
...
If local memory bytes are nonzero in activation or all_rows_direct:
  reduce register pressure or reject that fused variant.

If w_cache_read_model_ratio is much larger than 1.0:
...
fill_w_cache_ms is not a dominant fraction of the direct reference for qwen_shape_smallM
```
...
```text
Validate cache_first recompute from w_cache and dgate/dup math.
```

Implementation:

```text
Add recompute_grad_window path:
  grouped BF16 GEMM reads w_cache
  activation kernel adds LoRA-B delta and writes grad_pair_window
  grad_gate_sel/grad_up_sel are filled
```
...
grad_pair_window, grad_gate_sel, grad_up_sel pass tolerance:
  max_abs <= 3e-2
  max_rel <= 3e-2
...
```text
recompute_grad_window_ms is recorded
recompute_reads_w_cache_bytes > 0
local_memory_load_bytes and local_memory_store_bytes are reported
pair_acc_global_bytes is reported
NCU fields may be empty in Stage 2; focused NCU tensor-utilization and
activation-kernel local-memory checks are blocking only in Stage 7
```
...
```bash
python scripts/lf/validate_qwen3_gate_up_windowed_bwd.py \
  --stage stage2_recompute_activation \
  --op recompute_activation \
  --device cuda:0 \
  --cases ragged_groups,partial_window,qwen_shape_smallM \
  --profile-mode cuda-events \
  --output-dir profiling/qwen3_gate_up_windowed_bwd/stage2_recompute_activation
```
...
```bash
python scripts/lf/validate_qwen3_gate_up_windowed_bwd.py \
  --stage stage3_dx_window \
  --op dx_window \
  --device cuda:0 \
  --cases ragged_groups,partial_window,qwen_shape_smallM,qwen_shape_routed \
  --profile-mode cuda-events \
  --output-dir profiling/qwen3_gate_up_windowed_bwd/stage3_dx_window
```
...
```text
Validate full native pybind against torch reference without Python model integration.
```

Implementation:

```text
Expose qwen3_gate_up_recompute_bwd_sm100_bf16_windowed.
...
python scripts/lf/validate_qwen3_gate_up_windowed_bwd.py \
  ...
  --warmup-iters 10 \
  --latency-iters 50 \
  --output-dir profiling/qwen3_gate_up_windowed_bwd/stage4_native_direct_e2e
```
...
```text
Route selected recompute rows through the native path in qwen3_moe.py.
```
...
For dropout 0.00, recompute or pass S_gate/S_up deterministically.
...
median backward-region latency is recorded
no regression above 10 percent versus current selected path for the direct test
```
...
```bash
python scripts/lf/validate_qwen3_gate_up_windowed_bwd.py \
  --stage stage5_python_integration_drop000 \
  --op python_integration \
  --device cuda:0 \
  --lora-dropout 0.00 \
  --profile-mode cuda-events \
  --output-dir profiling/qwen3_gate_up_windowed_bwd/stage5_python_integration_drop000
```
...
```text
Prove the native path uses saved S_gate/S_up and does not own dropout.
```
...
```text
When lora_dropout > 0:
  selected S_gate/S_up must come from saved forward state
  native function receives S only
  no dropout masks or RNG enter the native function
```

Correctness gate:

```text
same trainable LoRA grad tolerances as Stage 5
native_kernel_consumed_saved_s == true
native_kernel_consumed_dropout_masks == false
...
```text
drop010 native-region latency is recorded
drop010 median direct latency <= 1.10 * drop000 same-shape latency
no extra native kernel launches for dropout handling
```
...
```text
Stage 4, Stage 5, and Stage 6 validation JSONs are passed
...
NSYS ranges exist:
  ...
  qwen3_gate_up_windowed/recompute_grad_window
  qwen3_gate_up_windowed/dx_window
NCU reports:
  dram bytes read/write for fill_w_cache
  HBM/L2 stats for w_cache reads
  tensor-core utilization for recompute and dX
  local memory bytes for activation and any fused epilogue
...
qwen_shape_smallM and one large/routed case
...
recompute.ncu-rep and recompute.csv
activation_backward.ncu-rep and activation_backward.csv
...
```text
all Stage 4 direct correctness cases pass
all Stage 5/6 integration correctness cases pass
cpu_weight_stream_multiplier <= 1.01
```
...
local-memory and pair_acc spill counters do not increase materially
```
...
pair_acc_global_bytes and local-memory bytes are below the measured saved HBM read
...
For the enabled run it should set:
...
Only after Stage 10 passes should broad E2E/product tile tuning begin. Limited
...
Do not tune before Stage 4-6 correctness passes and Stage 7 has produced an
...
```text
1. Increase BM before changing the algorithm.
   ...
   occupancy and local-memory spill remain acceptable.

2. Tune Q for dX efficiency and launch count.
...
4. Reduce launch/runtime overhead with CUDA graph capture or one native
   orchestration API.
   Hypothesis: phase launches are acceptable only when captured; uncaptured
   overhead should trigger native orchestration, not LF E2E.
   Test: NSYS reports launch count, CPU runtime overhead, and captured versus
   uncaptured timing.

5. If cache_first_window is correct but recompute w_cache reads dominate, try
   seed_group_direct.
...
6. Use persistent role-separated fusion only after Stage 7 proves launch/sync or
...
NSYS schedule checks, and NCU metric gates may be tested with LF E2E.
...
NCU reports are present for fill, recompute, activation, and dX
NCU shows no unexpected local memory spill or HBM byte explosion
stage4 native median latency improves or stays within 2 percent
stage5/stage6 integration gates still pass
stage10 full-step median improves before merging
```
