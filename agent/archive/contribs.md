...
Forward:
gate = X W_gate^T + LoRA_gate # offload to CPU as gate_tmp
up   = X W_up^T   + LoRA_up # offload to CPU as up_tmp
act  = silu(gate) * up # offload to CPU as act_tmp
...
dA_down = dS_down^T act_tmp # AsymGEMM

sig = sigmoid(gate) # CPU
silu_gate = gate * sig
silu_grad_gate = sig * (1 + gate * (1 - sig))

dgate = dact * up_tmp2 * silu_grad_gate # AsymGEMM
...
###  Forward
```
X = routed_rows_for_this_expert                      # [M, H] X_cpu: HBM
...
dgate = stage(dgate_cpu)
dX_gate_base = dgate @^ W_gate_cpu                        # [M, H] Temp
dX = dX_gate_base
...
# ---------------- up backward ----------------
dup = stage(dup_cpu)
dX_up_base = dup @^ W_up_cpu                              # [M, H] Temp
dX += dX_up_base
