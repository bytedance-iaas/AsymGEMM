...
set +e
guard T3_350
echo "=== T3''''b(dualda,ohbm9,blockexp) @350K (w1+m1) $(date -u +%H:%M:%S)"
...
WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
  bash scripts/lf/tp_probe.sh mixtral-8x22b m6mxf8t3350 "asym_cpuadamwds|recomp-off-full-fg-ker000-ceil0000-ohbm9|ligerloss1" 350000 1
T3EXIT=$?
echo "R13_T3_EXIT=$T3EXIT"
if [ $T3EXIT -ne 0 ]; then echo "PROBE13_STATE=t3_dead_350"; exit 3; fi
guard T2B_350
echo "=== T2B(fix2) @350K capacity attempt (w1+m1) $(date -u +%H:%M:%S)"
ASYM_EXACT_PINNED=1 ASYM_EXACT_PINNED_ROOTS=1 ASYM_EXACT_PINNED_SAVED=1 \
WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
  bash scripts/lf/tp_probe.sh mixtral-8x22b m6mxf8t2b350 "asym_cpuadamwds|T2B|ligerloss1" 350000 1
T2BEXIT=$?
echo "R13_T2B_EXIT=$T2BEXIT"
if [ $T2BEXIT -ne 0 ]; then echo "PROBE13_WINNER=350000"; exit 0; fi
echo "PROBE13_STATE=both_fit_350"
exit 3
