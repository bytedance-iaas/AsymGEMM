...
t3run() { # $1 = seq
  ...
  WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
    bash scripts/lf/tp_probe.sh mixtral-8x22b "m6mxg3t3$(( $1 / 1000 ))" "asym_cpuadamwds|recomp-off-full-fg-ker000-ceil0000-ohbm56|ligerloss1" "$1" 1
}
t2brun() { # $1 = seq
  guard "T2B_$1"
  echo "=== T2B(slab) @$1 capacity attempt (w1+m1) $(date -u +%H:%M:%S)"
  env $XPC WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
    bash scripts/lf/tp_probe.sh mixtral-8x22b "m6mxg3t2b$(( $1 / 1000 ))" "asym_cpuadamwds|T2B|ligerloss1" "$1" 1
}
pair() { # 0 winner / 1 t3-dead / 2 both-fit
  if t3run "$1"; then
    if t2brun "$1"; then echo "PAIR@$1: BOTH FIT"; return 2
    else echo "PROBE17_WINNER=$1"; return 0; fi
...
if [ $rc -eq 2 ]; then
  pair 400000; rc=$?
  [ $rc -eq 0 ] && exit 0
  echo "PROBE17_STATE=rc$rc@400"; exit 3
fi
pair 368000; rc=$?
[ $rc -eq 0 ] && exit 0
echo "PROBE17_STATE=rc$rc@368"
exit 3
