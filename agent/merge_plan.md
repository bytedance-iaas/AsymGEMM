...
## MERGE PROCEDURE (staged; do not reorder)
...
## VERIFICATION CHECKLIST (must pass TWICE in a row; log runs below)
  ...
  [ ] Syntax: `bash -n` on every changed/new .sh (run_lf_lora_sft.sh,
      profile_lora_lf_test_{both,source}.sh, ceiling_search_{both,source}.sh,
      ep_balance_bench.sh) and `python -m py_compile` on every changed .py
      (ceiling_search.py, ceiling_table.py, build_lf_sft_eval_pair.py, plus
      our four training files and the testing scripts) — all exit 0.
