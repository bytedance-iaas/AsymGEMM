...
1. **`attn_math.md` is aspirational — dense attention activation-offload is NOT wired.**
   ...
   Impact (Qwen3-32B, M=16384, bf16): clean projection win = `x_lora [M,H]` ≈ **160 MiB/attn-block** (qkv input, recompute-able from the stream) + the low-rank `S` saves. NB `AttnOut [M,Dq]` (256) is FlashAttention's saved `O` (needed in FA-bwd `D=rowsum(dO∘O)`); attn_proj dA_o reuses it on-GPU → removable only via FA-tensor offload (later stage), not a clean projection fetch. Current code (`dense.py:428`) materializes AttnOut as a separate `.contiguous()` copy distinct from FA's saved O — a removable 256 MiB duplicate.
...
4. **Hidden fp32 silu transient in the MLP.**
   `MLP.forward` does `activated = F.silu(gate.float()) * up.float()` (`asym_gemm/training/dense.py:471`) — transiently upcasts gate and up to fp32 `[M,I]`, ~1.6 GiB **each** at I=25600, M=16384. Must be counted in offload accounting; the win only lands if silu runs in the saved bf16 dtype or in fp32 on CPU (off-HBM). Backward compounds it: GPU silu-bwd restages gate,up to HBM and allocates `dgate_up [M,2I]` (~1.6 GiB) → MLP-bwd transient peak ≈ 4×`[M,I]`, even though gate,up are "offloaded" in the forward schedule.
...
- Wire attention-projection activation offload→fetch (`X` → CPU; LoRA-A via `@^L`) — issue 1.
- Choose recompute-vs-fetch for the MLP `act[M,I]` intermediate (GPU silu + stream, not CPU silu) — issues 3,4.
- Long-T: offload FlashAttention-saved Q/K/V/O (incl. AttnOut) as a later stage.
