...
# --- dev pairs (w1+m1) ---
MAX_STEPS=1 run_cell g2dev47u $FLASH "superoffload_mem|unsloth" 8000 "1"
MAX_STEPS=1 run_cell g2dev47a $FLASH "asym_cpuadamwds|T1"       8000 "1"
MAX_STEPS=1 run_cell g2dev45u $AIR   "superoffload_mem|unsloth" 8000 "1"
MAX_STEPS=1 run_cell g2dev45a $AIR   "asym_cpuadamwds|T1"       8000 "1"
echo "GLM-DEV-DONE $(date +%H:%M)" >> "$S"

# --- Flash validation: 192k·b2 pair, walker down on OOM, b3 capacity probes ---
o2=$(run_cell g2val47o $FLASH "$UNSOFF" 192000 "2 1")
t2=$(run_cell g2val47t $FLASH "$T3TOK"  192000 "2 1")
if [ "$o2" = "TRAINED" ]; then run_cell g2val47o3 $FLASH "$UNSOFF" 192000 "3"; fi
if [ "$t2" = "TRAINED" ]; then run_cell g2val47t3 $FLASH "$T3TOK"  192000 "3"; fi
echo "GLM-FLASH-VAL-DONE $(date +%H:%M)" >> "$S"

# --- Air validation: 128k·b2 pair, walker down on OOM, b3 capacity probes ---
oa=$(run_cell g2val45o $AIR "$UNSOFF" 128000 "2 1")
ta=$(run_cell g2val45t $AIR "$T3TOK"  128000 "2 1")
if [ "$oa" = "TRAINED" ]; then run_cell g2val45o3 $AIR "$UNSOFF" 128000 "3"; fi
if [ "$ta" = "TRAINED" ]; then run_cell g2val45t3 $AIR "$T3TOK"  128000 "3"; fi
