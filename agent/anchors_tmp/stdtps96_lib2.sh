#!/bin/bash
# stdtps96_lib2.sh — METHOD V2 (PEAK-AUDIT) run library, c11 (standardize_
# tps_96gb.md "METHOD V2"): cells run WITHOUT the occupier on the full card;
# the 96G verdict comes from the EXACT post-run peak reserved HBM.
...
# Termination ladder + shm hygiene per OPS LESSONS. No occupier checks.
...
run_cell_pa() { local tag="$1" model="$2" systok="$3" seq="$4" blist="$5" policy="${6:-none|false|false|false|false|false}" ranks="${7:-1}" v=SKIP b
  for b in $blist; do
    ...
    RUN_NAME="${tag}-g96c11_${model}" RUNS="${model}|${ranks} ; ${systok}|ligerloss1 ; ${seq}|${b}|1 ; ${policy}" \
      bash scripts/lf/profile_lora_lf_test_source.sh >> "$LOGD/r_${tag}_b${b}.log" 2>&1
    local dmodel=${model//./_}
    v=$(verdict_pa "${tag}-g96c11_${dmodel}__b${b}_s${seq}_ga1_drop000" "$LOGD/r_${tag}_b${b}.log")
