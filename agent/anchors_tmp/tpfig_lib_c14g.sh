...
# c14 PORT (gpt-oss-120b campaign 2026-08-15) of the SFT-39 template (run_glms.md §4): paths -> SFT-46 tree,
# RUN_NAME/dirname suffix -c14g_, NUMACTL pinning added (c18 house protocol;
...
verdict() { local dtag="$1" log="$2"
  ...
  # completeness-checker quirk (qwen3.5/gpt-oss classes): jobs.tsv says failed
  # but the run finished — trust step_samples with a completed measured step.
  local d="$B/${dtag}"
...
run_cell() { local tag="$1" model="$2" systok="$3" seq="$4" blist="$5" policy="${6:-none|false|false|false|false|false}" ranks="${7:-1}" v=SKIP b
  for b in $blist; do
    ...
    RUN_NAME="${tag}-c14g_${model}" RUNS="${model}|${ranks} ; ${systok}|ligerloss1 ; ${seq}|${b}|1 ; ${policy}" \
      bash scripts/lf/profile_lora_lf_test_source.sh >> "$LOGD/r_${tag}_b${b}.log" 2>&1
    local dmodel=${model//./_}
    v=$(verdict "${tag}-c14g_${dmodel}__b${b}_s${seq}_ga1_drop000" "$LOGD/r_${tag}_b${b}.log")
