...
set -uo pipefail
export GPU="${GPU:-0}" HOSTFLOOR="${HOSTFLOOR:-500}"
...
T1="asym_cpuadamwds|T1"
T2B="asym_cpuadamwds|T2B"
...
LADDER="32000 64000 128000 192000 256000 320000 384000 448000 512000 640000 768000 896000"
blist_for() { case "$1" in 32000|64000) echo "2 1" ;; *) echo "1" ;; esac; }

# ---- smoke: integration proof (asym T1 @32k b1) ----
note "G1 begin; smoke T1@32k"
v=$(run_cell g120smk "$MODEL" "$T1" 32000 "1" "$POL" 1)
note "SMOKE -> $v"
[ "$v" = "TRAINED" ] || { note "SMOKE FAILED — halting for diagnosis"; exit 1; }

# ---- baseline walls ----
for pair in "rc:$RC" "un:$UNS" "uo:$UO"; do
  name="${pair%%:*}"; tok="${pair#*:}"
  for seq in $LADDER; do
    v=$(run_cell "g120${name}$((seq/1000))" "$MODEL" "$tok" "$seq" "$(blist_for $seq)" "$POL" 1)
...
for seq in $LADDER; do
  if [ "$tier" = "T3" ]; then
    v=$( (export "${T3ENV[@]}"; run_cell "g120a$((seq/1000))" "$MODEL" "$T3TOK" "$seq" "1" "$POL" 1) )
  else
    v=$(run_cell "g120a$((seq/1000))" "$MODEL" "$tok" "$seq" "$(blist_for $seq)" "$POL" 1)
  fi
  note "CELL asym[$tier] s=$seq -> $v"
  if [ "$v" = "GOOM" ] || [ "$v" = "COOM" ]; then
    case "$tier" in
      T1)  tier=T2B; tok="$T2B"; note "tier -> T2B at $seq"
           v=$(run_cell "g120a$((seq/1000))b" "$MODEL" "$tok" "$seq" "1" "$POL" 1); note "CELL asym[T2B] s=$seq -> $v" ;;
      T2B) tier=T3; note "tier -> T3 at $seq"
           v=$( (export "${T3ENV[@]}"; run_cell "g120a$((seq/1000))c" "$MODEL" "$T3TOK" "$seq" "1" "$POL" 1) ); note "CELL asym[T3] s=$seq -> $v" ;;
