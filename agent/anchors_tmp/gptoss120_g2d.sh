...
T1="asym_sepplanlink2_cpuadamwds|unsloth-ohbm4"
...
note "G2D T1-ohbm4 + gradoff ladder (roots partially HBM-resident)"

# ---- baseline walls ----
# ---- asym T1-ohbm4 ladder (no fallthrough; host-targeted) ----
for seq in $LADDER; do
  v=$(run_cell "g2ha$((seq/1000))" "$MODEL" "$T1" "$seq" "1" "$POL" 2)
  note "CELL asym[T1o4] s=$seq -> $v"
  if [ "$v" = "GOOM" ] || [ "$v" = "COOM" ]; then note "asym T1o4 WALL at $seq"; break; fi
  [ "$v" = "FAIL" ] && { note "asym T1o4 FAIL at $seq"; break; }
done
note "G2D-DONE"
