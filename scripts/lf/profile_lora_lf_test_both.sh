...
LF_DIR=${LF_DIR:-${ROOT}/third-party/LlamaFactory}
...
LF_FA4_DIR=${LF_FA4_DIR:-${ROOT}/third-party/LlamaFactory}
LF_DS_CONFIG_DIR=${LF_DS_CONFIG_DIR:-${ROOT}/third-party/LlamaFactory/examples/deepspeed}
KT_KERNEL_DIR=${KT_KERNEL_DIR:-${ROOT}/third-party/ktransformers/kt-kernel}
DEEPSPEED_DIR=${DEEPSPEED_DIR:-${ROOT}/third-party/deepspeed}
...
if [[ "${_RUNS_ENV_SET}" == "true" ]]; then
  ...
  RUNS=(
    # "q3-30b-a3b|1 ; superoffload_mem|unsloth-ohbm0|ligerloss1 ; 20000|8|1 ; none|false|false|false|false|false"
    "q3-30b-a3b|1 ; asym_cpuadamwds|recomp-off-full-fg-ker101-ceil0000-ohbm0|ligerloss1 ; 20000|8|1 ; none|false|false|false|false|false"
  )
fi
...
# Accumulated across all scheduled runs for the end-of-sweep peak-HBM comparison table.
_PEAK_SUMMARY_LABELS=()
_PEAK_SUMMARY_PJ=()
...
run_job() {
  ...
  # Capture (config label, profile.json) for the end-of-sweep peak-HBM table.
  _PEAK_SUMMARY_LABELS+=("${run_log_payload}")
  _PEAK_SUMMARY_PJ+=("${profile_json}")
...
# ---- Peak allocated HBM comparison for the scheduled RUNS configs ----
if ((${#_PEAK_SUMMARY_LABELS[@]})); then
  echo ""
  echo "===== Peak allocated HBM per config ====="
  _peak_data=""
  for _psi in "${!_PEAK_SUMMARY_LABELS[@]}"; do
    _peak_data+="${_PEAK_SUMMARY_LABELS[$_psi]}"$'\t'"${_PEAK_SUMMARY_PJ[$_psi]}"$'\n'
  done
  _PEAK_DATA="${_peak_data}" python3 - <<'PY' || true
import os, json
rows = {}
for line in os.environ.get("_PEAK_DATA", "").splitlines():
    if not line:
        continue
    label, _, pj = line.partition("\t")
    peak = None
    try:
        b = json.load(open(pj)).get("memory", {}).get("peak_allocated_hbm_bytes")
        if b:
            peak = b / (2 ** 30)
    except Exception:
        pass
    if label not in rows or (peak is not None and (rows[label] is None or peak > rows[label])):
        rows[label] = peak
w = max((len(l) for l in rows), default=1)
for label, peak in rows.items():
    val = ("%6.2f GiB" % peak) if peak is not None else "n/a"
    print("  %-*s   peak_alloc_HBM = %s" % (w, label, val))
PY
  echo "========================================="
fi
