...
# ── TIER PRESETS (read this before hand-writing env) ─────────────────────────
# RUNS accepts backend|TIER (T1 / T2 / T2B / T3) in the backend slot, e.g.
#   RUNS="q3-30b-a3b|1 ; asym_cpuadamwds|T2B ; 800000|1|1 ; none|..."
# The preset expands (tier_expand_backend_spec below) from scripts/lf/
# tier_recipes.sh — GENERATED by asym_scheduler.py --emit-recipes; never edit
# it by hand. A tier carries the recompute token + the 6 MoE pins + the CPU
# stack (ASYM_PLACEMENT_POLICY=1 ASYM_CPU_OPS_THREADS=48; self-gating policy,
# see agent/impls/placement.md). Raw hand-written token specs get NONE of that
# (code defaults are all OFF by design) — you'd silently benchmark the
# pre-CPU-merge tree. User env still wins over recipe env (the A/B hook).
# Picking tier+batch for a (model, seq): asym_scheduler.py MODEL SEQ.
...
declare -A M=(
  ...
  [mixtral-8x22b]="/scratch_local/user_data/shutian/kevin/cache/fused/Mixtral-8x22B-v0.1" # layers: 56; FUSED local copy 2026-08-06 (bit-identical weights, pre-applied expert fusion -> no load-time conversion transient; was mistralai/Mixtral-8x22B-v0.1)
...
# GLM family predicate (fix_glm_t3.md 2026-08-08): also used for the GLM-only
# attn LoRA-A cpu-left reroute env in run_job (segfault-class fix; see
# attention_activation_offload._dense_lora_a_cpu_left).
is_glm_family_model() {
  case "$1" in
...
# HY_T3 campaign; mixtral MX_T3 same day; GLM shapes numerics-validated in
# fix_glm_t3.md — 4way-merge union 2026-08-09). Phi stays outside until
# validated the same way. Deliberately SEPARATE from is_qwen3_moe_routed_model
# so the ker101 AUTO-DEFAULT (qwen3_moe_routed_auto_default) stays qwen-only
# and bare `recomp-off-full-fg` tokens keep their historical ker000 meaning
# for every other family.
is_route_kernel_capable_model() {
  is_qwen3_moe_routed_model "$1" && return 0
  is_glm_family_model "$1" && return 0
  case "$1" in
...
validate_recompute_kernel_for_model() {
  ...
  if [[ "${kernel_code}" != "000" ]] && ! is_route_kernel_capable_model "${model}"; then
    die "recomp-off-full-fg-ker${kernel_code} is only supported for Qwen3/Qwen3.5/Hunyuan/Mixtral routed MoE + the GLM family (fix_glm_t3.md); model '${model}' must use recomp-off-full-fg-ker000"
  fi
}
...
run_job() {
  ...
  if qwen3_route_any_enabled "${q3rt_fwd_flag}" "${q3rt_gather_flag}" "${q3rt_dx_flag}" "${q3rt_lora_flag}"; then
    ...
    is_route_kernel_capable_model "${current_model_name}" || die "Qwen3 routed kernels are scoped to Qwen3/Qwen3.5/Hunyuan/Mixtral routed MoE + the GLM family (fix_glm_t3.md), got model='${current_model_name}'"
