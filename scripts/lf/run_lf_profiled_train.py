...
def _config_from_args(args: list[str]) -> dict[str, Any]:
    ...
    is_superoffload_backend = backend in {"superoffload", "superoffload_mem", "superoffload_mem_opnvme", "superoffload_mem_panvme"}
...
def _superoffload_summary_from_config(config: dict[str, Any]) -> dict[str, Any]:
    is_superoffload_backend = str(config.get("backend") or "").lower() in {"superoffload", "superoffload_mem", "superoffload_mem_opnvme", "superoffload_mem_panvme"}
...
def _should_install_deepspeed_hook(args: list[str], config: dict[str, Any]) -> bool:
    ...
    if backend in {"zero2", "zero3", "zero3_offload", "zero3_offload_mem", "zero3_offload_opnvme", "zero3_offload_panvme", "zero3_offload_mem_opnvme", "zero3_offload_mem_panvme", "zero3_cpuadam", "superoffload", "superoffload_mem", "superoffload_mem_opnvme", "superoffload_mem_panvme"}:
        return True
