...
# BACKEND_SPECS=${BACKEND_SPECS:-"zero3_offload|recomp|ligerloss1"}
...
# # Full finetuning by default for this script. lora|full. When full, the runner emits
# # --finetuning_type full and drops all --lora_* args; LORA_* below are then unused.
...
#     torch|zero2|zero3|zero3_offload|zero3_offload_mem|zero3_offload_opnvme|zero3_cpuadam|superoffload|superoffload_mem) printf '%s\n' "${model_gpu_count}" ;;
...
# is_zero_backend() {
#   case "${1}" in
#     zero2|zero3|zero3_offload|zero3_offload_mem|zero3_offload_opnvme|zero3_cpuadam|superoffload|superoffload_mem) return 0 ;;
#     *) return 1 ;;
#   esac
# }
...
#   case "${1}" in
#     torch|zero2|zero3|zero3_offload|zero3_offload_mem|zero3_offload_opnvme|zero3_cpuadam|superoffload|superoffload_mem|kt_*) return 0 ;;
#     *) return 1 ;;
#   esac
# }
...
#   # Full finetuning records no lora_target; LoRA runs pin it to "all".
#   local expected_lora_target="all"
#   [[ "${FINETUNING_TYPE:-lora}" == "full" ]] && expected_lora_target=""
...
#   *) die "FINETUNING_TYPE must be lora or full, got '${FINETUNING_TYPE}'" ;;
# esac
...
#     zero2|zero3|zero3_offload|zero3_offload_mem|zero3_offload_opnvme|zero3_cpuadam) selected_has_zero=true ;;
