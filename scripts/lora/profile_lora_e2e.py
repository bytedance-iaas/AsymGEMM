...
def is_hf_model_workload(workload: str) -> bool:
    return "/" in workload


def is_moe_workload_name(workload: str) -> bool:
...
def apply_expert_recompute_policy_label(args: argparse.Namespace) -> None:
    value = getattr(args, "expert_recompute_policy", None)
    raw = "none" if value is None else str(value).strip()
    try:
        spec = parse_expert_recompute_policy_spec(raw)
    except ValueError as exc:
        raise ValueError(
            f"invalid --expert-recompute-policy {raw!r}; expected none, tok-le0, tok-le0-act, tok-leN, tok-geN, tokA-B, or -act variants"
        ) from exc
    args.expert_recompute_policy = spec.policy
    args.expert_recompute_threshold = spec.token_threshold
    args.expert_recompute_token_min = spec.token_min
    args.expert_recompute_token_max = spec.token_max
    args.expert_recompute_policy_spec = spec.label
    args.expert_activation_save_policy = spec.activation_save_policy
    args.expert_activation_save_threshold = spec.activation_save_threshold
    args.expert_activation_save_token_min = spec.activation_save_min
    args.expert_activation_save_token_max = spec.activation_save_max
    args.expert_policy_label = spec.label
    args.expert_force_custom_autograd = spec.force_custom_autograd
...
def profile_layer_count(args: argparse.Namespace, *, max_layers: int) -> int:
    ...
    try:
        ...
        raise ValueError("--profile-layers must be a positive integer or 'all'") from exc
    if value <= 0:
        raise ValueError("--profile-layers must be positive")
    return max(1, min(value, int(max_layers)))


def validate_backend_workload(args: argparse.Namespace) -> None:
    expert_policy = normalize_expert_recompute_policy(getattr(args, "expert_recompute_policy", "none"))
    expert_threshold = normalize_expert_recompute_threshold(getattr(args, "expert_recompute_threshold", 0))
    expert_token_min = normalize_expert_recompute_threshold(getattr(args, "expert_recompute_token_min", 1))
    expert_token_max = getattr(args, "expert_recompute_token_max", None)
    expert_token_max = None if expert_token_max is None else normalize_expert_recompute_threshold(expert_token_max)
    ...
    activation_save_min = normalize_expert_activation_save_threshold(getattr(args, "expert_activation_save_token_min", 1))
    activation_save_max = getattr(args, "expert_activation_save_token_max", None)
    activation_save_max = (
        None if activation_save_max is None else normalize_expert_activation_save_threshold(activation_save_max)
    )
    expert_recompute_active = expert_recompute_policy_enabled(
        expert_policy,
        token_threshold=expert_threshold,
        token_min=expert_token_min,
        token_max=expert_token_max,
    )
    expert_activation_save_active = expert_activation_save_policy_enabled(
        ...
        token_min=activation_save_min,
        token_max=activation_save_max,
    )
    ...
    if expert_activation_save_active and not is_moe_workload_name(args.workload):
        raise ValueError("--expert-recompute-policy *-act is only supported for MoE LoRA SFT workloads.")
    if is_kt_backend(args.backend) and expert_activation_save_active:
        raise ValueError("backend=kt does not support --expert-recompute-policy *-act")
...
class StageBook:
    def __init__(self, device: torch.device, *, timing_mode: str = "profile") -> None:
        ...
        self.route_count_records: list[dict[str, Any]] = []
    ...
    def clear(self) -> None:
        ...
        self.max_stage_peak_bytes = 0
        self.route_count_records.clear()
    ...
    def record_route_metadata(
        ...
        *,
        ...
        expert_recompute_token_min: int = 1,
        expert_recompute_token_max: int | None = None,
        expert_recompute_policy_spec: str = "",
        expert_recompute_impl: str = "none",
        expert_activation_save_policy: str = "save_all",
        expert_activation_save_threshold: int = 0,
        expert_activation_save_token_min: int = 1,
        expert_activation_save_token_max: int | None = None,
        ...
        if not isinstance(counts_tensor, torch.Tensor):
            return
        counts_cpu = counts_tensor.detach().to(device="cpu", dtype=torch.long)
        counts = [int(v) for v in counts_cpu.tolist()]
        active_counts = [value for value in counts if value > 0]
        ...
        token_min = normalize_expert_recompute_threshold(expert_recompute_token_min)
        token_max = (
            None
            if expert_recompute_token_max is None
            else normalize_expert_recompute_threshold(expert_recompute_token_max)
        )
        ...
        activation_min = normalize_expert_activation_save_threshold(expert_activation_save_token_min)
        activation_max = (
            None
            if expert_activation_save_token_max is None
            else normalize_expert_activation_save_threshold(expert_activation_save_token_max)
        )
        ...
        activation_drop_mask = expert_activation_drop_group_mask(
            ...
            token_min=activation_min,
            token_max=activation_max,
        ) & ~recompute_mask
        ...
        keep_utils = [value for count, value, flag in zip(counts, utils, recompute_flags) if count > 0 and not flag]
        if not expert_recompute_policy_spec:
            policy_active = expert_recompute_policy_enabled(
                policy,
                token_threshold=threshold,
                token_min=token_min,
                token_max=token_max,
            ) or expert_activation_save_policy_enabled(
                activation_policy,
                token_threshold=activation_threshold,
                token_min=activation_min,
                token_max=activation_max,
            ...
            if policy_active:
                raise ValueError("expert_recompute_policy_spec is required for active expert policies")
        ...
        self.route_count_records.append(
            {
                ...
                "top_k": int(getattr(metadata, "top_k", 0)),
                "num_experts": int(getattr(metadata, "num_experts", len(counts))),
                "expert_recompute_policy": policy,
                "expert_recompute_threshold": threshold,
                "expert_recompute_token_min": token_min,
                "expert_recompute_token_max": token_max,
                "expert_recompute_policy_spec": str(expert_recompute_policy_spec),
                "expert_recompute_impl": str(expert_recompute_impl or "none"),
                "expert_activation_save_policy": activation_policy,
                "expert_activation_save_threshold": activation_threshold,
                "expert_activation_save_token_min": activation_min,
                "expert_activation_save_token_max": activation_max,
                "expert_policy_label": str(expert_policy_label),
                "counts": counts,
                "paid_rows": paid_rows,
                "utils": utils,
                "total_routes": int(sum(counts)),
                "active_experts": int(len(active_counts)),
                "empty_experts": int(len(counts) - len(active_counts)),
                "recomputed_experts": int(len(recompute_counts)),
                ...
                "kept_util_avg": _mean_float(keep_utils),
            }
        )

    def route_summary(self) -> dict[str, Any]:
        records = self.route_count_records
        if not records:
            return {"samples": 0, "per_expert": []}
        counts_by_sample = [list(record["counts"]) for record in records]
        num_experts = max(len(counts) for counts in counts_by_sample)
        padded_counts = [counts + [0] * (num_experts - len(counts)) for counts in counts_by_sample]
        all_counts = [value for counts in padded_counts for value in counts]
        active_counts = [value for value in all_counts if value > 0]
        per_expert: list[dict[str, Any]] = []
        for expert_idx in range(num_experts):
            expert_values = [counts[expert_idx] for counts in padded_counts]
            per_expert.append(
                {
                    "expert": expert_idx,
                    "avg_tokens": _mean_float(expert_values),
                    "median_tokens": _median_float(expert_values),
                    "min_tokens": _min_int(expert_values),
                    "max_tokens": _max_int(expert_values),
                }
            )
        first = records[0]
        policy = str(first.get("expert_recompute_policy", "tok"))
        policy_spec = str(first.get("expert_recompute_policy_spec", "none"))
        expert_recompute_impl = str(first.get("expert_recompute_impl", "none"))
        ...
        kept_utils = [float(record.get("kept_util_avg", 0.0)) for record in records]
        active_experts = [int(record["active_experts"]) for record in records]
        return {
            "samples": len(records),
            "mode": str(first.get("mode", "")),
            "num_tokens": int(first.get("num_tokens", 0)),
            ...
            "threshold_effect": {
                "samples_with_recompute": int(sum(1 for value in recomputed_experts if value > 0)),
                "samples_all_active_recomputed": int(
                    sum(1 for rec, active in zip(records, active_experts) if active > 0 and int(rec["kept_experts"]) == 0)
                ),
                "recomputed_experts_avg": _mean_float(recomputed_experts),
                "recomputed_experts_min": _min_int(recomputed_experts),
                "recomputed_experts_max": _max_int(recomputed_experts),
                **{f"recomputed_experts_{key}": value for key, value in _standard_percentiles(recomputed_experts).items()},
                "kept_experts_avg": _mean_float(kept_experts),
                "kept_experts_min": _min_int(kept_experts),
                "kept_experts_max": _max_int(kept_experts),
                **{f"kept_experts_{key}": value for key, value in _standard_percentiles(kept_experts).items()},
                "recomputed_routes_avg": _mean_float(recomputed_routes),
                "recomputed_routes_min": _min_int(recomputed_routes),
                "recomputed_routes_max": _max_int(recomputed_routes),
                **{f"recomputed_routes_{key}": value for key, value in _standard_percentiles(recomputed_routes).items()},
                "kept_routes_avg": _mean_float(kept_routes),
                ...
                **{f"recomputed_paid_rows_{key}": value for key, value in _standard_percentiles(recomputed_paid_rows).items()},
                "kept_paid_rows_avg": _mean_float(kept_paid_rows),
                "kept_paid_rows_min": _min_int(kept_paid_rows),
                "kept_paid_rows_max": _max_int(kept_paid_rows),
                **{f"kept_paid_rows_{key}": value for key, value in _standard_percentiles(kept_paid_rows).items()},
                "recomputed_util_avg": _mean_float(recomputed_utils),
                "kept_util_avg": _mean_float(kept_utils),
            },
...
def _mean_float(values: list[int] | list[float]) -> float:
    if not values:
        return 0.0
    return float(sum(values)) / float(len(values))


def _median_float(values: list[int] | list[float]) -> float:
    if not values:
        return 0.0
...
def _percentile_float(values: list[int] | list[float], percentile: float) -> float:
    if not values:
        return 0.0
    ordered = sorted(float(v) for v in values)
    if len(ordered) == 1:
        return ordered[0]
    rank = max(0.0, min(100.0, float(percentile))) / 100.0 * float(len(ordered) - 1)
    left = int(math.floor(rank))
    right = int(math.ceil(rank))
    if left == right:
        return ordered[left]
    weight = rank - float(left)
    return ordered[left] * (1.0 - weight) + ordered[right] * weight


def _standard_percentiles(values: list[int] | list[float]) -> dict[str, float]:
    return {
        "p0": _percentile_float(values, 0),
        "p25": _percentile_float(values, 25),
        "p50": _percentile_float(values, 50),
        "p75": _percentile_float(values, 75),
        "p90": _percentile_float(values, 90),
        "p100": _percentile_float(values, 100),
    }


def _token_stats(values: list[int] | list[float]) -> dict[str, float | int]:
    return {
        "avg": _mean_float(values),
        "median": _median_float(values),
        "min": _min_int([int(v) for v in values]),
        "max": _max_int([int(v) for v in values]),
        **_standard_percentiles(values),
    }
...
class _ProfiledRMSNorm(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx: Any,
        x: torch.Tensor,
        ...
        out = x_f * rstd * weight_f
        ctx.save_for_backward(x_f, rstd, weight_f)
        ctx.prefix = prefix
        ctx.book = book
        return out.to(dtype=x.dtype)
    ...
    def backward(ctx: Any, grad_output: torch.Tensor) -> tuple[torch.Tensor | None, None, None, None, None]:
        ...
        with ctx.book.time(f"backward.{ctx.prefix}.grad"):
            grad_norm = grad_output.float() * weight
            projection = (grad_norm * x).mean(dim=-1, keepdim=True) * rstd.pow(2)
            grad_x = (grad_norm - x * projection) * rstd
        return grad_x.to(dtype=grad_output.dtype), None, None, None, None


def profiled_frozen_layer_norm(module: torch.nn.Module, x: torch.Tensor, prefix: str, book: StageBook) -> torch.Tensor:
    if hasattr(module, "frozen_bias"):
        return _ProfiledLayerNorm.apply(x, module.frozen_weight, module.frozen_bias, float(module.eps), prefix, book)
    return _ProfiledRMSNorm.apply(x, module.frozen_weight, float(module.eps), prefix, book)
...
def patch_dense_forward(book: StageBook) -> list[tuple[Any, str, Any]]:
    ...
    original_attn = dense.SelfAttention.forward
    ...
    def timed_attn(self: Any, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        with book.time(f"{prefix}.{attention_impl}"):
            context = scaled_dot_product_attention(q, k, v)
        _attach_backward_nvtx_ranges(context, f"backward.{bprefix}.{attention_impl}", book, stop_tensors=(q, k, v))
    ...
    originals.append((dense.SelfAttention, "forward", original_attn))
    dense.SelfAttention.forward = timed_attn

    original_mlp = dense.MLP.forward
    ...
    originals.append((dense.MLP, "forward", original_mlp))
    dense.MLP.forward = timed_mlp

    original_layer = dense.DecoderLayer.forward
    ...
    originals.append((dense.DecoderLayer, "forward", original_layer))
    dense.DecoderLayer.forward = timed_layer

    original_base = dense.DenseLLMBase.forward
    ...
    originals.append((dense.DenseLLMBase, "forward", original_base))
    dense.DenseLLMBase.forward = timed_base
    return originals
...
def dense_forward_keys(num_layers: int, target_names: set[str], offload_names: set[str], attention_impl: str) -> list[str]:
    ...
    for layer_idx in range(num_layers):
        ...
        keys.append(f"forward.{layer}.attention.{attention_impl}")
        keys.extend(
            dense_projection_forward_keys(
                layer,
                "attention",
...
def dense_backward_prefixes(num_layers: int, target_names: set[str], offload_names: set[str], attention_impl: str) -> list[str]:
        ...
        keys.extend(
            [
                ...
                *dense_projection_backward_prefixes(
                ...
                f"backward.{layer}.attention.{attention_impl}",
                *dense_projection_backward_prefixes(
                    layer,
                    "attention",
...
def profile_dense(args: argparse.Namespace, device: torch.device, dtype: torch.dtype) -> dict[str, Any]:
    ...
    stats = AsymExecutionStats()
    weights = make_dense_weights(config, seed=1, dtype=dtype)
    if args.backend == "torch":
        model = TorchDenseLLM(
            weights,
            config=config,
    ...
    return build_report(
        ...
        avg,
        stats.as_dict(),
        forward_keys=dense_forward_keys(config.num_layers, target_names, offload_names, args.attention_impl),
        backward_group_prefixes=dense_backward_prefixes(config.num_layers, target_names, offload_names, args.attention_impl),
...
def patch_moe_forward(book: StageBook) -> list[tuple[Any, str, Any]]:
    ...
    original_grouped_compact = moe.AsymMoELayer._run_grouped_compact_body
    ...
    originals.append((moe.AsymMoELayer, "_run_grouped_compact_body", original_grouped_compact))
    moe.AsymMoELayer._run_grouped_compact_body = timed_grouped_compact

    original_attn = moe.SelfAttention.forward
    ...
    def timed_attn(self: Any, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        attention_impl = self.attention_impl
        with book.time(f"forward.attention.{attention_impl}"):
            context = scaled_dot_product_attention(q, k, v)
        _attach_backward_nvtx_ranges(context, f"backward.attention.{attention_impl}", book, stop_tensors=(q, k, v))
        context = context.transpose(1, 2).contiguous().view(batch, seq, hidden)
    ...
    originals.append((moe.SelfAttention, "forward", original_attn))
    moe.SelfAttention.forward = timed_attn
    ...
    def timed_layer_forward(self: Any, x: torch.Tensor, *, static_routing: Any = None, mode: str = "contiguous", return_details: bool = False) -> Any:
        ...
        if isinstance(details, dict) and book.current_range() == "step.forward":
            book.record_route_metadata(
    ...
    for layer_cls in (moe.AsymMoELayer, moe.TorchMoELayer, moe.KTMoELayer):
        originals.append((layer_cls, "forward", layer_cls.forward))
        layer_cls.forward = timed_layer_forward

    original_run_moe = moe.AsymMoELayer._run_moe
    ...
    originals.append((moe.AsymMoELayer, "_run_moe", original_run_moe))
    moe.AsymMoELayer._run_moe = timed_run_moe

    original_kt_run_moe = moe.KTMoELayer._run_moe
    ...
    originals.append((moe.KTMoELayer, "_run_moe", original_kt_run_moe))
    moe.KTMoELayer._run_moe = timed_kt_run_moe

    original_model_forward = moe.MoE.forward
    ...
    originals.append((moe.MoE, "forward", original_model_forward))
    moe.MoE.forward = timed_model_forward
    return originals


def moe_forward_keys(config: Any, attention_impl: str) -> list[str]:
    keys = [
        "forward.embeddings",
        ...
        f"forward.attention.{attention_impl}",
...
def moe_backward_prefixes(config: Any, attention_impl: str) -> list[str]:
    keys = [
        "backward.loss.mse",
        "backward.final_norm",
    ...
    keys.extend(
        [
            ...
            f"backward.attention.{attention_impl}",
...
def profile_moe(args: argparse.Namespace, device: torch.device, dtype: torch.dtype) -> dict[str, Any]:
    ...
    config_extra = dict(getattr(args, "_config_extra", {}))
    lora_dtype = profile_lora_dtype(args)
    expert_recompute_policy = normalize_expert_recompute_policy(getattr(args, "expert_recompute_policy", "none"))
    expert_recompute_threshold = normalize_expert_recompute_threshold(getattr(args, "expert_recompute_threshold", 0))
    expert_recompute_token_min = normalize_expert_recompute_threshold(getattr(args, "expert_recompute_token_min", 1))
    expert_recompute_token_max = getattr(args, "expert_recompute_token_max", None)
    expert_recompute_token_max = (
        None if expert_recompute_token_max is None else normalize_expert_recompute_threshold(expert_recompute_token_max)
    )
    expert_recompute_active = expert_recompute_policy_enabled(
        expert_recompute_policy,
        token_threshold=expert_recompute_threshold,
        token_min=expert_recompute_token_min,
        token_max=expert_recompute_token_max,
    )
    ...
    expert_activation_save_token_min = normalize_expert_activation_save_threshold(
        getattr(args, "expert_activation_save_token_min", 1)
    )
    expert_activation_save_token_max = getattr(args, "expert_activation_save_token_max", None)
    expert_activation_save_token_max = (
        None
        if expert_activation_save_token_max is None
        else normalize_expert_activation_save_threshold(expert_activation_save_token_max)
    )
    expert_activation_save_active = expert_activation_save_policy_enabled(
        ...
        token_min=expert_activation_save_token_min,
        token_max=expert_activation_save_token_max,
    )
    ...
    if is_kt_backend(args.backend) and expert_activation_save_active:
        raise ValueError("backend=kt does not support --expert-recompute-policy *-act")
    expert_recompute_policy_spec = str(getattr(args, "expert_recompute_policy_spec", ""))
    if not expert_recompute_policy_spec:
        if expert_recompute_active or expert_activation_save_active:
            raise ValueError("expert_recompute_policy_spec is required for active expert policies")
        expert_recompute_policy_spec = "none"
    ...
    config_extra["expert_recompute_token_min"] = expert_recompute_token_min
    config_extra["expert_recompute_token_max"] = expert_recompute_token_max
    config_extra["expert_recompute_policy_spec"] = expert_recompute_policy_spec
    config_extra["expert_recompute_impl"] = "none"
    ...
    config_extra["expert_activation_save_token_min"] = expert_activation_save_token_min
    config_extra["expert_activation_save_token_max"] = expert_activation_save_token_max
    ...
    state = None
    qwen_moe_workload = str(getattr(args, "workload", ""))
    if not is_hf_model_workload(qwen_moe_workload) or str(config_extra.get("model_type", "")) != QWEN_MOE_MODEL_TYPE:
        qwen_moe_workload = ""
    if qwen_moe_workload:
        config = replace(config, num_layers=max(1, int(config.num_layers)))
        config_extra["profiled_layers"] = int(config.num_layers)
        config_extra["weight_source"] = "hf-qwen_moe_block"
        config_extra["hf_layer_index"] = int(getattr(args, "hf_layer_index", 0))
        state = qwen_moe_block_state_from_hf(config, args, model_id=qwen_moe_workload, seed=3, base_dtype=dtype)
    else:
        config_extra.setdefault("weight_source", "synthetic")
    if state is None:
    ...
    moe_route_pattern = str(getattr(args, "moe_route_pattern", "balanced"))
    if moe_route_pattern == "learned":
        static_routes = None
    else:
        static_routes = make_static_routes(config, device, pattern=moe_route_pattern)
    config_extra["moe_route_pattern"] = moe_route_pattern
    batch_seed = int(getattr(args, "profile_seed", 1234))
    batch_generator = torch.Generator(device=device if device.type == "cuda" else "cpu")
    ...
    return build_report(
        ...
        avg,
        stats.as_dict(),
        forward_keys=moe_forward_keys(config, args.attention_impl),
        backward_group_prefixes=moe_backward_prefixes(config, args.attention_impl),
        config={**asdict(config), "moe_mode": args.moe_mode, **config_extra},
        stage_memory=book.memory_summary(),
        memory_attribution=memory_attribution_report(model, optimizer, device, book.saved_tensor_tracker),
        expert_token_distribution=book.route_summary(),
    )
...
def build_report(
    ...
    *,
    ...
    expert_token_distribution: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "expert_token_distribution": expert_token_distribution or {"samples": 0, "per_expert": []},
...
def markdown(report: dict[str, Any]) -> str:
    ...
    if isinstance(rows, list) and rows:
        ...
        lines.append("")
    route_stats = report.get("expert_token_distribution", {})
    if isinstance(route_stats, dict) and int(route_stats.get("samples", 0) or 0) > 0:
        all_tokens = route_stats.get("all_expert_tokens", {})
        active_tokens = route_stats.get("active_expert_tokens", {})
        active_experts = route_stats.get("active_experts", {})
        ...
        lines.extend(
            [
                ...
                f"| expert recompute impl | {str(route_stats.get('expert_recompute_impl', 'none'))} |",
                ...
                f"| all-expert p50 tokens | {float(all_tokens.get('p50', all_tokens.get('median', 0.0))):.2f} |",
                f"| all-expert p75 tokens | {float(all_tokens.get('p75', 0.0)):.2f} |",
                f"| all-expert p90 tokens | {float(all_tokens.get('p90', 0.0)):.2f} |",
                f"| all-expert p100 tokens | {float(all_tokens.get('p100', all_tokens.get('max', 0.0))):.2f} |",
                f"| active-expert avg tokens | {float(active_tokens.get('avg', 0.0)):.2f} |",
                f"| active-expert p0 tokens | {float(active_tokens.get('p0', active_tokens.get('min', 0.0))):.2f} |",
                f"| active-expert p25 tokens | {float(active_tokens.get('p25', 0.0)):.2f} |",
                f"| active-expert p50 tokens | {float(active_tokens.get('p50', active_tokens.get('median', 0.0))):.2f} |",
                f"| active-expert p75 tokens | {float(active_tokens.get('p75', 0.0)):.2f} |",
                f"| active-expert p90 tokens | {float(active_tokens.get('p90', 0.0)):.2f} |",
                f"| active-expert p100 tokens | {float(active_tokens.get('p100', active_tokens.get('max', 0.0))):.2f} |",
                f"| active experts avg | {float(active_experts.get('avg', 0.0)):.2f} |",
                f"| active experts p0 | {float(active_experts.get('p0', active_experts.get('min', 0.0))):.2f} |",
                f"| active experts p50 | {float(active_experts.get('p50', active_experts.get('median', 0.0))):.2f} |",
                f"| active experts p100 | {float(active_experts.get('p100', active_experts.get('max', 0.0))):.2f} |",
                f"| samples with expert recompute | {int(threshold_effect.get('samples_with_recompute', 0))} |",
                f"| samples all active experts recomputed | {int(threshold_effect.get('samples_all_active_recomputed', 0))} |",
                f"| recomputed experts avg | {float(threshold_effect.get('recomputed_experts_avg', 0.0)):.2f} |",
                f"| recomputed experts p50 | {float(threshold_effect.get('recomputed_experts_p50', threshold_effect.get('recomputed_experts_avg', 0.0))):.2f} |",
                f"| recomputed experts p90 | {float(threshold_effect.get('recomputed_experts_p90', 0.0)):.2f} |",
                f"| recomputed routes avg | {float(threshold_effect.get('recomputed_routes_avg', 0.0)):.2f} |",
                f"| recomputed routes p50 | {float(threshold_effect.get('recomputed_routes_p50', threshold_effect.get('recomputed_routes_avg', 0.0))):.2f} |",
                f"| recomputed routes p90 | {float(threshold_effect.get('recomputed_routes_p90', 0.0)):.2f} |",
                f"| recomputed paid rows avg | {float(threshold_effect.get('recomputed_paid_rows_avg', 0.0)):.2f} |",
                f"| recomputed util avg | {float(threshold_effect.get('recomputed_util_avg', 0.0)):.3f} |",
                f"| kept experts avg | {float(threshold_effect.get('kept_experts_avg', 0.0)):.2f} |",
                f"| kept experts p50 | {float(threshold_effect.get('kept_experts_p50', threshold_effect.get('kept_experts_avg', 0.0))):.2f} |",
                f"| kept experts p90 | {float(threshold_effect.get('kept_experts_p90', 0.0)):.2f} |",
        ...
        per_expert = route_stats.get("per_expert", [])
        if isinstance(per_expert, list) and per_expert:
            lines.extend(
                [
                    "### Per-Expert Routed Tokens",
                    "",
                    "| Expert | avg | median | min | max |",
                    "|---:|---:|---:|---:|---:|",
                ]
            )
            for row in per_expert:
                if not isinstance(row, dict):
                    continue
                lines.append(
                    f"| {int(row.get('expert', 0))} | "
                    f"{float(row.get('avg_tokens', 0.0)):.2f} | "
                    f"{float(row.get('median_tokens', 0.0)):.2f} | "
                    f"{int(row.get('min_tokens', 0))} | "
                    f"{int(row.get('max_tokens', 0))} |"
                )
            lines.append("")
...
def moe_config_from_metadata(metadata: dict[str, Any], args: argparse.Namespace) -> Any:
    from asym_gemm.training.moe import MoEConfig
...
def _download_hf_file(model_id: str, filename: str, *, cache_dir: str | None, local_files_only: bool) -> Path:
    try:
        from huggingface_hub import hf_hub_download
    except Exception as exc:
        raise ImportError("Qwen MoE workloads require huggingface_hub") from exc
    return Path(
        hf_hub_download(
            model_id,
            filename,
...
def _hf_weight_map(model_id: str, *, cache_dir: str | None, local_files_only: bool) -> tuple[Path, dict[str, str]]:
    ...
    return index_path.parent, {str(name): str(filename) for name, filename in weight_map.items()}


def _hf_model_workload_label(model_id: str) -> str:
    return re.sub(r"[^A-Za-z0-9_-]+", "_", model_id).strip("_") or "hf_model"


def _hf_config_json(model_id: str, args: argparse.Namespace) -> dict[str, Any]:
    cache_dir = str(args.hf_cache_dir) if getattr(args, "hf_cache_dir", None) else None
    local_files_only = bool(getattr(args, "hf_local_files_only", False))
    config_path = _download_hf_file(model_id, "config.json", cache_dir=cache_dir, local_files_only=local_files_only)
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if not isinstance(config, dict):
        raise ValueError(f"{model_id} config.json did not contain an object")
    return config


def _required_hf_int(hf_config: dict[str, Any], key: str, *, model_id: str) -> int:
    try:
        return int(hf_config[key])
    except KeyError as exc:
        raise ValueError(f"{model_id} config.json is missing required key {key!r}") from exc
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{model_id} config.json key {key!r} must be an integer") from exc


def hf_qwen_moe_metadata_from_model_id(model_id: str, args: argparse.Namespace) -> dict[str, Any]:
    hf_config = _hf_config_json(model_id, args)
    model_type = str(hf_config.get("model_type", ""))
    if model_type != QWEN_MOE_MODEL_TYPE:
        raise ValueError(
            f"HF workload {model_id!r} has model_type={model_type!r}; "
            f"this profiler currently extracts Qwen MoE blocks with model_type={QWEN_MOE_MODEL_TYPE!r}."
        )
    moe_intermediate_size = _required_hf_int(hf_config, "moe_intermediate_size", model_id=model_id)
    return {
        "model_type": model_type,
        "num_hidden_layers": _required_hf_int(hf_config, "num_hidden_layers", model_id=model_id),
        "hidden_size": _required_hf_int(hf_config, "hidden_size", model_id=model_id),
        "intermediate_size": int(hf_config.get("intermediate_size", moe_intermediate_size)),
        "moe_intermediate_size": moe_intermediate_size,
        "num_attention_heads": _required_hf_int(hf_config, "num_attention_heads", model_id=model_id),
        "num_experts": _required_hf_int(hf_config, "num_experts", model_id=model_id),
        "num_experts_per_tok": _required_hf_int(hf_config, "num_experts_per_tok", model_id=model_id),
        "num_shared_experts": int(hf_config.get("num_shared_experts", 0) or 0),
        "vocab_size": _required_hf_int(hf_config, "vocab_size", model_id=model_id),
    }


def _synthetic_state_for_hf_moe_block(config: Any, *, seed: int, base_dtype: torch.dtype) -> dict[str, Any]:
    ...
    def randn(shape: tuple[int, ...], std: float, *, dtype: torch.dtype = torch.float32) -> torch.Tensor:
        return (torch.randn(shape, generator=generator, dtype=torch.float32) * std).to(dtype=dtype)

    def lora_expert() -> dict[str, torch.Tensor]:
        return {
            "gate_lora_a": randn((rank, hidden), 0.01),
            "gate_lora_b": randn((inter, rank), 0.01),
            "up_lora_a": randn((rank, hidden), 0.01),
            "up_lora_b": randn((inter, rank), 0.01),
            "down_lora_a": randn((rank, inter), 0.01),
            "down_lora_b": randn((hidden, rank), 0.01),
        }

    def layer_state() -> dict[str, Any]:
        return {
            "q_proj": randn((hidden, hidden), 0.02, dtype=base_dtype),
            "k_proj": randn((hidden, hidden), 0.02, dtype=base_dtype),
            "v_proj": randn((hidden, hidden), 0.02, dtype=base_dtype),
            "o_proj": randn((hidden, hidden), 0.02, dtype=base_dtype),
            "input_layernorm_weight": torch.ones(hidden, dtype=base_dtype),
            "post_attention_layernorm_weight": torch.ones(hidden, dtype=base_dtype),
            "norm_type": "rmsnorm",
            "rms_norm_eps": 1e-6,
            "router_weight": torch.empty((int(config.num_experts), hidden), dtype=base_dtype),
            "experts": [lora_expert() for _ in range(int(config.num_experts))],
            "shared_experts": [],
        }

    return {
        "token_embedding": randn((int(config.vocab_size), hidden), 0.02, dtype=base_dtype),
        "position_embedding": randn((max(int(config.logical_tokens), int(config.seq_len)), hidden), 0.02, dtype=base_dtype),
        "lm_head": randn((int(config.vocab_size), hidden), 0.02, dtype=base_dtype),
        "final_layernorm_weight": torch.ones(hidden, dtype=base_dtype),
        "layers": [layer_state() for _ in range(int(config.num_layers))],
    }


def qwen_moe_block_state_from_hf(
    config: Any,
    args: argparse.Namespace,
    *,
    model_id: str,
    ...
    layer_idx = int(args.hf_layer_index)
    cache_dir = str(args.hf_cache_dir) if getattr(args, "hf_cache_dir", None) else None
    local_files_only = bool(getattr(args, "hf_local_files_only", False))
    config_path = _download_hf_file(model_id, "config.json", cache_dir=cache_dir, local_files_only=local_files_only)
    ...
    expected = {
        ...
        "num_experts_per_tok": int(config.top_k),
    }
    for key, value in expected.items():
        hf_value = int(hf_config.get(key, -1))
        if hf_value != value:
            raise ValueError(
                f"{model_id} {key}={hf_value} does not match workload {key}={value}; "
                "the resolved HF config and extracted block shape must match."
            )
    num_hidden_layers = int(hf_config.get("num_hidden_layers", 0) or 0)
    num_profile_layers = int(config.num_layers)
    if layer_idx < 0 or (num_hidden_layers > 0 and layer_idx >= num_hidden_layers):
        raise ValueError(f"--hf-layer-index must be in [0, {num_hidden_layers}), got {layer_idx}")
    if num_hidden_layers > 0 and layer_idx + num_profile_layers > num_hidden_layers:
        raise ValueError(
            f"requested {num_profile_layers} Qwen MoE layers from --hf-layer-index {layer_idx}, "
            f"but {model_id} has {num_hidden_layers} layers"
        )
    snapshot_dir, weight_map = _hf_weight_map(model_id, cache_dir=cache_dir, local_files_only=local_files_only)

    names: list[str] = []
    hf_layer_indices = list(range(layer_idx, layer_idx + num_profile_layers))
    for hf_layer_idx in hf_layer_indices:
        layer_prefix = f"model.layers.{hf_layer_idx}"
        names.extend(
            [
    ...
    if missing:
        raise KeyError(f"missing Qwen MoE block tensors in {model_id}: {missing[:5]}")

    state = _synthetic_state_for_hf_moe_block(config, seed=seed, base_dtype=base_dtype)
    layer_index_to_local = {hf_layer_idx: local_idx for local_idx, hf_layer_idx in enumerate(hf_layer_indices)}
    for layer_state in state["layers"]:
        layer_state["rms_norm_eps"] = float(hf_config.get("rms_norm_eps", 1e-6))
    by_file: dict[str, list[str]] = defaultdict(list)
    for name in names:
        by_file[weight_map[name]].append(name)

    def checked(name: str, tensor: torch.Tensor, expected_shape: torch.Size | tuple[int, ...]) -> torch.Tensor:
        value = tensor.detach().to(dtype=base_dtype).contiguous()
    ...
    def assign(name: str, tensor: torch.Tensor) -> None:
        layer_match = re.search(r"model\.layers\.(\d+)\.", name)
        if layer_match is None:
            raise KeyError(name)
        local_layer_idx = layer_index_to_local.get(int(layer_match.group(1)))
        if local_layer_idx is None:
            raise KeyError(name)
        layer_state = state["layers"][local_layer_idx]
        if name.endswith(".input_layernorm.weight"):
            layer_state["input_layernorm_weight"] = checked(name, tensor, layer_state["input_layernorm_weight"].shape)
        elif name.endswith(".post_attention_layernorm.weight"):
            layer_state["post_attention_layernorm_weight"] = checked(name, tensor, layer_state["post_attention_layernorm_weight"].shape)
        elif name.endswith(".mlp.gate.weight"):
            layer_state["router_weight"] = checked(name, tensor, layer_state["router_weight"].shape)
        else:
            match = re.search(r"\.experts\.(\d+)\.(gate_proj|up_proj|down_proj)\.weight$", name)
            if match is None:
                raise KeyError(name)
            expert_idx = int(match.group(1))
            projection = match.group(2)
            state_key = {"gate_proj": "gate_weight", "up_proj": "up_weight", "down_proj": "down_weight"}[projection]
            expected_shape = (int(config.intermediate_size), int(config.hidden_size))
            if projection == "down_proj":
                expected_shape = (int(config.hidden_size), int(config.intermediate_size))
            layer_state["experts"][expert_idx][state_key] = checked(name, tensor, expected_shape)

    for filename, file_names in by_file.items():
        path = snapshot_dir / filename
        if not path.exists():
            path = _download_hf_file(model_id, filename, cache_dir=cache_dir, local_files_only=local_files_only)
        with safe_open(str(path), framework="pt", device="cpu") as handle:
            for name in file_names:
                assign(name, handle.get_tensor(name))
...
def profile_hf_model_workload(
    ...
    *,
    ...
    metadata = hf_qwen_moe_metadata_from_model_id(model_id, args)
    return profile_moe_metadata_workload(
        args,
        device,
        dtype,
        workload_name=_hf_model_workload_label(model_id),
        metadata=metadata,
    )
...
def run_workload(args: argparse.Namespace, device: torch.device, dtype: torch.dtype) -> dict[str, Any]:
    ...
    if is_hf_model_workload(args.workload):
        return profile_hf_model_workload(
            args,
            device,
            dtype,
            model_id=args.workload,
        )
    ...
    allowed = ", ".join(WORKLOAD_CHOICES)
    raise ValueError(f"unknown workload {args.workload!r}; use one of {allowed}, or pass an HF model id containing '/'.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--workload",
        default="mlp",
        help=f"Built-in workload ({', '.join(WORKLOAD_CHOICES)}) or HF model id containing '/'.",
    )
    ...
    parser.add_argument(
        ...
        default="sdpa",
        help="Attention implementation. fa2/fa3/fa4 are placeholders until their kernels are wired.",
    )
    ...
    parser.add_argument(
        ...
        default="none",
        help="MoE expert policy: none, tok-le0, tok-leN, tok-geN, tokA-B, or -act variants.",
    )
        ...
        json.dumps(
            {
                ...
                "step_ms": report["step"]["total_milliseconds"],
                "expert_token_distribution": report.get("expert_token_distribution", {"samples": 0, "per_expert": []}),
            },
            indent=2,
        )
    )
