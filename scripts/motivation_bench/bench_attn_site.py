...
def main() -> None:
    ...
    dual_native = getattr(asym_gemm, "sm100_grouped_lora_a_dual_bf16_cpu_right", None)
    s_fp32 = torch.zeros((N, RANK), device=dev, dtype=torch.float32)

    def dual_one():
        # N2: ONE X stream -> S AND dA (vs two-pass = fwd + dA = 2 streams).
        s_fp32.zero_()
        g = torch.empty((1, RANK, D), device=dev, dtype=torch.bfloat16)
        dual_native(ds3[0], u, b3[0], s_fp32, g, offsets, experts, 2)
        return s_fp32.to(torch.bfloat16), g[0]

    def twopass_one():
        sv = cpu_left_impl.grouped_expert_lora_cpu_left(u, b3[0], offsets, experts)
        g = torch.empty((1, RANK, D), device=dev, dtype=torch.bfloat16)
        grad_native(ds3[0], u, g, offsets, experts, 2)
        return sv, g[0]
    ...
    fns = {
        ...
        "dual_one": (dual_one, N * D * 2),
        "twopass_one": (twopass_one, 2 * N * D * 2),
    ...
    if triple_native is not None:
        ...
        fns.pop("fwd_triple")
    if dual_native is not None:
        sd, gd = dual_one()
        st, gt2 = twopass_one()
        torch.cuda.synchronize()
        checks["dual_S_vs_twopass"] = rel(sd, st)
        checks["dual_dA_vs_twopass"] = rel(gd, gt2)
        del sd, gd, st, gt2
    else:
        fns.pop("dual_one"); fns.pop("twopass_one")
    ...
    if "dual_one" in results and "twopass_one" in results:
        print(f"[summary] N2 dual-dataflow vs two-pass: {results['twopass_one']['mean_ms']/results['dual_one']['mean_ms']:.3f}x")
