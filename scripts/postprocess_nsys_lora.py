...
def _timing_summary_markdown(report: dict[str, Any], stage_name: str, title: str) -> list[str]:
    ...
    for row in _semantic_rows(stage):
...
def _overall_memory_markdown(source_profile: dict[str, Any]) -> list[str]:
    ...
    rows = [
        ...
        ("CPU host W_T", cpu.get("host_w_t_bytes", 0)),
        ("CPU pinned W", cpu.get("pinned_w_bytes", 0)),
        ("CPU pinned W_T", cpu.get("pinned_w_t_bytes", 0)),
...
def _stage_memory_markdown(source_profile: dict[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        "",
        "| Stage | samples | alloc start MiB | alloc end MiB | alloc delta MiB | reserved start MiB | reserved end MiB | reserved delta MiB | global peak after MiB |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
...
def _memory_attribution_markdown(profile: dict[str, Any]) -> list[str]:
    ...
    if isinstance(category_rows, list) and category_rows:
        lines.extend(["| Category | Memory space | bytes | MiB | Accuracy |", "|---|---|---:|---:|---|"])
        for row in category_rows:
            value = int(row.get("bytes", 0))
            lines.append(
                f"| {row.get('category', '-')} | {row.get('memory_space', '-')} | "
                f"{value} | {_fmt_mib(value)} | {row.get('accuracy', '-')} |"
            )
        lines.append("")
    else:
    ...
    if isinstance(saved_rows, list) and saved_rows:
        lines.extend(
            [
                ...
                "",
                "| Owner | unique bytes | unique MiB | reference bytes | reference MiB | saves | unique tensors |",
                "|---|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for row in saved_rows:
            ...
            lines.append(
                f"| {row.get('owner', '-')} | {unique} | {_fmt_mib(unique)} | "
