...
## q3.5-122b — FINAL (2026-07-20 17:0x). asym T2 @480k = HOST-OOM (watchdog 47<50).
**TIER-INVERSION FINDING: on host-bound models (234 GB pinned weights) keep-acts OFF
ADDS host load → T2/T3 infeasible where D binds → T1 IS the deepest tier.** The
scheduler's host-line feasibility screen (system_summary §2) catches exactly this —
measured validation of the D-screen's necessity.
FINAL 122B TABLE (cell = lat·TP·HBM(%)·RSS):
| seq | so-recomp | so-unsloth | so-unsloth-OFF | asym | verdict |
|---|---|---|---|---|---|
| 32k b8 | GPU-OOM | 281.8·909·154.0(83%)·659 | HOST-OOM | T1: 304.4·841·113.7(61%)·903 | asym −7.5%, −40 GiB; 2/3 baselines dead |
| 320k b1 | OOM* | 499.9·640·181.3(98%)·659 — last fit | OOM* | T1 est ~860 | uns edge-locked |
| 352k b1 | OOM* | **GPU-OOM → wall (320k,352k]** | OOM* | — | |
| 384k b1 | OOM* | OOM | OOM* | T1: 439.5·874·156.9(85%)·848 | **sole coverage, +37% over uns's best-alive** |
| 448k b1 | OOM* | OOM | OOM* | T1: 499.3·**897**·180.8(98%)·849 | **max-seq = 448k ≈ 1.36× uns wall, +40%** |
| 480k b1 | OOM* | OOM | OOM* | T1 GPU-OOM pred · **T2 HOST-OOM measured** | asym wall (tier-inverted) |
Plot: figures/out/tp_vs_seq_q3_5-122b-a10b.png (4-series, est cells bordered).
CAMPAIGN COMPLETE for both models. Next-lever if 122B depth matters: linear-attn
keep-acts port (remaining_optimizations 7b) — cuts host pools AND the −7.5% @32k;
then ohbm/actnvme host arbitrage.


## q3.5-122b addendum (2026-07-20 ~22:00): asym T1 @320k b1 = MEASURED HOST-DNF.
4 attempts (n512 x3 from both dirty and clean-idle starts, n256 x1) all watchdog-
killed at 46-49 GiB vs the 50 floor, while 384k/448k fit at RSS 848-849. The
288-320k band systematically exceeds the host floor for asym T1 on this node —
mechanism unresolved (pinned-pool shape quantization suspected; same host-pool
family as the linear-attn port, remaining_optimizations 7b). Cell recorded as
host-OOM (not est); the deep-end story rests on the measured 384k/448k points.
Final 122B table row: 320k asym = HOST-DNF · uns 640 (98% edge, last fit).
