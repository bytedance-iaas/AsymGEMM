...
**H2D side-stream v3 (post-measurement, flag `ASYM_NVME_ACT_H2D_STREAM=1`)** — the third rung of the same ladder, NOT shelved: with v2 hiding the NVMe→CPU leg, the remaining un-overlapped cost is the substrates' synchronous unpack CPU→HBM copy (~20 ms/GiB vs NVMe ~40 ms/GiB — roughly a third of total reload). Megatron/TE hide it with a dedicated H2D stream + double-buffered destinations + per-group events (`fine_grained_activation_offload.py:412-413,1083-1093`; `cpu_offload_v1.py:349-378`). Implement only after Stage-3 v1/v2 counters show `_unpack` H2D time matters; it also benefits plain CPU offload (independent of NVMe). Sequencing rationale: v1 fully sync = correct + bit-exact-testable + trivially debuggable; every overlap rung is flag-gated so debugging can always drop back to sync.
...
class BaseWeightPager:
    ...
    def _issue_prefetches(self) -> None:
        # Byte-budgeted lookahead (DS pattern, 0.6): uniform lead TIME regardless of blob-size mix
        # (matters for MoE: GB expert blobs interleaved with ~50MB attn weights). For dense models
        # this degenerates to a fixed count since per-layer shapes are uniform.
        if not self._frozen or self._disabled: return
        n = len(self._trace); d = 1; inflight = self._inflight_prefetch_bytes()
        while inflight < self.prefetch_bytes and d < n:      # ASYM_NVME_BASE_WEIGHT_PREFETCH_BYTES (default 2×largest blob)
            k = self._trace[(self._cursor + d) % n]; d += 1
            e = self._entries[k]
            if e.state is not ABSENT: continue
            # Tight-cache guard (DS max_live analog): if making room would evict a buffer with a
            # NEARER next use than this target, prefetching hurts by definition — stop.
            if self._would_evict_nearer_than(e): break
...
- ~~Side-stream H2D staging~~ — moved into Stage 3 as the flag-gated **v3** rung (post-measurement), not shelved; see Stage 3.
