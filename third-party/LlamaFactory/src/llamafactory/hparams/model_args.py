...
class KTransformersArguments:
    r"""Arguments pertaining to KTransformers MoE SFT training.
    ...
    use_kt: bool = field(
        default=False,
        metadata={"help": "Whether to use KTransformers MoE backend for SFT training."},
    )
    kt_backend: Literal[
        "AMXBF16",
        "AMXINT8",
        ...
        metadata={
            "help": (
                "KTransformers backend. AMX* backends require upstream transformers-kt/accelerate-kt; "
                "TORCHBF16 uses the local portable SFT oracle; ARMBF16 uses the native ARM BF16 SFT path."
            )
        },
    )
    kt_num_threads: int | None = field(
        default=None,
        metadata={"help": "Number of CPU worker threads used by the KT backend."},
    )
    kt_tp_enabled: bool | None = field(
        default=None,
        metadata={"help": "Whether to enable the KT threadpool path."},
    )
    kt_threadpool_count: int | None = field(
        default=None,
        metadata={"help": "Number of KT threadpools when kt_tp_enabled is true."},
    )
    kt_num_gpu_experts: int | None = field(
        default=None,
        metadata={"help": "Number of routed experts kept on GPU; Phase 1 ARM defaults to CPU experts."},
    )
    kt_max_cache_depth: int | None = field(
        default=None,
        metadata={"help": "Maximum saved KT MoE forward cache depth for backward."},
    )
    ...
    def get_kt_config_dict(self, finetuning_args: Any, model_max_length: int | None) -> dict[str, Any]:
        ...
        kt_config = {
            ...
            "kt_max_cache_depth": self.kt_max_cache_depth,
    ...
    def apply_kt_config(self, finetuning_args: Any, training_args: Any, model_max_length: int | None) -> None:
        ...
        setattr(self, "kt_model_max_length", model_max_length)
        env_mapping = {
...
@dataclass
class AsymGEMMArguments:
    r"""Arguments pertaining to AsymGEMM LoRA-SFT training."""

    use_asym_gemm: bool = field(
        default=False,
        metadata={"help": "Whether to use AsymGEMM LoRA-SFT backend."},
    )
    asym_backend: Literal["asym", "torch"] = field(
        default="asym",
        metadata={"help": "Routed expert base GEMM backend: asym or torch."},
    )
    asym_precision: Literal["bf16"] = field(
        default="bf16",
        metadata={"help": "AsymGEMM precision. First pass supports bf16 only."},
    )
    asym_offload_modules: str = field(
        default="routed_experts",
        metadata={"help": "Comma-separated frozen base weight buckets to CPU-offload in the AsymGEMM path."},
    )
    asym_expert_recompute_policy: str = field(
        default="none",
        metadata={"help": "AsymGEMM routed expert recompute policy: none, splitN, tokN-ckpt, or tokN-act-ckpt."},
    )
    asym_router_mode: Literal["hf", "whole"] = field(
        default="whole",
        metadata={"help": "AsymGEMM MoE router handling for Qwen3/Llama4: hf or whole."},
    )
    asym_strict: bool = field(
        default=True,
        metadata={"help": "Raise if requested AsymGEMM targets are missing or unsupported."},
    )
