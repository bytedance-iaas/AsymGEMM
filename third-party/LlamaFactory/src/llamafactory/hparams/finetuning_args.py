...
class FinetuningArguments(
    ...
    asym_cpu_adamw_grad_offload: bool = field(
        default=False,
        metadata={
            "help": "Offload AsymGEMM LoRA grads to CPU during backward instead of copying them in optimizer.step."
        },
    )
    asym_cpu_adamw_weight_offload: bool = field(
        default=False,
        metadata={
            "help": "JIT-stage expert-LoRA weights from pinned CPU to CUDA per layer and release after use "
            "(requires asym_cpu_adamw_grad_offload=true). Reduces peak HBM by removing resident expert-LoRA weights."
        },
    )
