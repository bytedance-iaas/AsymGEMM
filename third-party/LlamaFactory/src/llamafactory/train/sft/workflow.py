...
def run_sft(
    ...
    if model_args.use_asym_gemm:
        if training_args.predict_with_generate:
            raise NotImplementedError("`predict_with_generate` is not supported in AsymGEMM SFT yet.")
        elif finetuning_args.compute_accuracy:
            raise NotImplementedError("`compute_accuracy` is not supported in AsymGEMM SFT yet.")
    ...
    if training_args.do_train:
        ...
        if training_args.save_strategy != IntervalStrategy.NO:
            ...
            logger.info_rank0("Skipping final model save because save_strategy=no.")
        ...
        if training_args.save_strategy != IntervalStrategy.NO:
            ...
            logger.info_rank0("Skipping final trainer state save because save_strategy=no.")
