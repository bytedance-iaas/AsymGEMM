...
def create_modelcard_and_push(
    ...
    if model_args.use_asym_gemm:
        kwargs["tags"] = kwargs["tags"] + ["asym-gemm"]
...
def _create_asym_cpu_adamw_optimizer(
    ...
    named_lora = named_lora_parameters(model)
    trainable_lora = [(name, param) for name, param in named_lora if param.requires_grad]
    if not trainable_lora:
        raise ValueError("AsymGEMM CPU AdamW found no trainable LoRA parameters.")
    ...
    trainable_ids = {id(param) for _, param in trainable_named}
    trainable_lora_ids = {id(param) for _, param in trainable_lora}
    if trainable_ids != trainable_lora_ids:
        non_lora = [name for name, param in trainable_named if id(param) not in trainable_lora_ids]
        missing = [name for name, param in trainable_lora if id(param) not in trainable_ids]
        raise ValueError(
            "AsymGEMM CPU AdamW requires every trainable parameter to be LoRA. "
            f"trainable_non_lora={non_lora[:20]}, lora_not_trainable={missing[:20]}"
        )

    if torch.cuda.is_available():
        not_cuda = [(name, param.device) for name, param in trainable_lora if param.device.type != "cuda"]
        if not_cuda:
            details = ", ".join(f"{name} on {device}" for name, device in not_cuda[:8])
            raise ValueError(
                "AsymGEMM CPU AdamW v1 requires trainable LoRA compute params to be CUDA nn.Parameters. "
                "LlamaFactory's CPU-first Asym load must call _move_asym_cpu_first_model_to_device() after "
                f"init_adapter(...). CPU-resident trainable LoRA is Stage 7. Offending params: {details}"
            )
    ...
    if finetuning_args.asym_cpu_adamw_weight_offload:
        # Build + install the coordinator AFTER the optimizer so its fp32 masters are captured from
        # the full GPU weights; install_lora_weight_offload then captures bf16 homes and releases.
        from asym_gemm.training.weight_offload import (
        ...
        if installed <= 0:
            raise ValueError(
                "`asym_cpu_adamw_weight_offload=true` but no Asym-owned LoRA weights were found to offload."
            )
        optimizer.attach_weight_offload_coordinator(coordinator)
        summary = coordinator.summary()
        logger.info_rank0(
            "Installed AsymGEMM LoRA weight offload on "
            f"{installed} trainable LoRA banks across {summary.get('weight_offload_group_count', 0)} groups. "
            f"Groups by component: {summary.get('weight_offload_group_count_by_component', {})}. "
            f"Skipped bytes by component: {summary.get('weight_offload_skipped_bytes_by_component', {})}."
        )
    summary = optimizer.asym_cpu_adamw_summary()
    compact_summary = {key: value for key, value in summary.items() if key not in {"param_names", "alias_param_names"}}
    logger.info_rank0(f"Using AsymGEMM CPU AdamW optimizer: {compact_summary}")
    return optimizer
