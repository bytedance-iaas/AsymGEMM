...
sm100_bf16_gemm_impl(int* grouped_layout,
    ...
    if (warp_idx == 0 and cute::elect_one_sync()) {
        ...
        if (lane_idx == 0 and blockIdx.x == 0 and blockIdx.y == 0) {
            printf("DBG_BF16_GEMM UMMA_DESC_ARGS "
                   "kMajorA=%u kMajorB=%u "
                   "BLOCK_M=%u BLOCK_N=%u BLOCK_K_=%u BLOCK_K=%u "
                   "kNumStages_=%u kNumStagesPerMerge=%u kNumStages=%u "
                   "LOAD_BLOCK_M=%u LOAD_BLOCK_N=%u BLOCK_ATOM_K=%u "
                   "kSwizzleAMode=%u kSwizzleBMode=%u\n",
                   (unsigned)kMajorA, (unsigned)kMajorB,
                   (unsigned)BLOCK_M, (unsigned)BLOCK_N, (unsigned)BLOCK_K_, (unsigned)BLOCK_K,
                   (unsigned)kNumStages_, (unsigned)kNumStagesPerMerge, (unsigned)kNumStages,
                   (unsigned)LOAD_BLOCK_M, (unsigned)LOAD_BLOCK_N, (unsigned)BLOCK_ATOM_K,
                   (unsigned)kSwizzleAMode, (unsigned)kSwizzleBMode);
        }
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0) {
                    ...
                    printf("DBG_BF16_GEMM LOAD_2x2 k_block_idx=%d, A=[[%.6f, %.6f],[%.6f, %.6f]] "
                           "B=[[%.6f, %.6f],[%.6f, %.6f]] stage=%u\n",
                           k_block_idx,
                           bf16_to_float(smem_a[stage_idx][0]),
                           bf16_to_float(smem_a[stage_idx][1]),
                           bf16_to_float(smem_a[stage_idx][a_row_stride]),
                           bf16_to_float(smem_a[stage_idx][a_row_stride + 1]),
                           bf16_to_float(smem_b[0][0]),
                           bf16_to_float(smem_b[0][1]),
                           bf16_to_float(smem_b[0][b_row_stride]),
                           bf16_to_float(smem_b[0][b_row_stride + 1]),
                           (unsigned)stage_idx);
                }
                    ...
                    for (uint32_t i = 0; i < STORE_BLOCK_N / kNumElemsPerBankGroup; ++ i) {
                        ...
                        if constexpr (cute::is_same_v<cd_dtype_t, float>) {
                            ...
                            if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0 and w == 0 and s == 0 and i == 0 and blockIdx.y == 0) {
                                printf("DBG_BF16_GEMM FP32 EPI_TMEM warp=%u epi_warp=%u lane=%u stage=%u tmem_addr=%u values=%f,%f,%f,%f\n",
                            ...
                            cutlass::arch::fence_view_async_tmem_load();
                            if (lane_idx == 0 and m_block_idx == 0 and n_block_idx == 0 and w == 0 and s == 0 and i == 0 and blockIdx.y == 0) {
                                printf("DBG_BF16_GEMM BF16 EPI_TMEM warp=%u epi_warp=%u lane=%u stage=%u tmem_addr=%u values=%f,%f,%f,%f\n",
                                       (unsigned)warp_idx, (unsigned)epilogue_warp_idx, (unsigned)lane_idx, (unsigned)accum_stage_idx, (unsigned)tmem_addr,
                                       __uint_as_float(values[0]), __uint_as_float(values[1]),
                                       __uint_as_float(values[2]), __uint_as_float(values[3]));
                            }
