...
sm90_int8_asym_gemm_1d1d_impl(uint32_t* offsets, uint32_t* experts,
                              uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    // Types: INT8 inputs, int32 (S32) accumulators
    using WGMMA = typename INT8MMASelector<BLOCK_N>::type;
    using Barrier = cutlass::arch::ClusterTransactionBarrier;

    // INT8 asym GEMM on SM90 requires FP32 output
    DG_STATIC_ASSERT(cute::is_same_v<cd_dtype_t, float>, "INT8 asym GEMM on SM90 requires FP32 output");
    ...
    DG_STATIC_ASSERT(BLOCK_K % WGMMA::K == 0, "BLOCK_K must be multiple of WGMMA K for INT8");
    ...
    // FP32 output uses no CD swizzle (kSwizzleCDMode == 0): a single TMA store covers
    // the whole BLOCK_N, and smem_cd is laid out row-major with pitch BLOCK_N.
    constexpr uint32_t CD_ROW_BYTES = (kSwizzleCDMode == 0) ? (BLOCK_N * sizeof(cd_dtype_t)) : kSwizzleCDMode;
    constexpr uint32_t STORE_BLOCK_N = CD_ROW_BYTES / sizeof(cd_dtype_t);
    ...
    // Shared memory sizes
    constexpr uint32_t SMEM_CD_SIZE_PER_STAGE = STORE_BLOCK_M * CD_ROW_BYTES;
    constexpr uint32_t SMEM_CD_SIZE = SMEM_CD_SIZE_PER_STAGE * kNumTMAStoreStages;
    constexpr uint32_t SMEM_A_SIZE_PER_STAGE = LOAD_BLOCK_M * BLOCK_K * sizeof(int8_t);
    constexpr uint32_t SMEM_B_SIZE_PER_STAGE = LOAD_BLOCK_N * BLOCK_K * sizeof(int8_t);
    ...
    if (warp_idx >= kNumMathThreads / 32) {
        ...
        if (warp_idx == kNumMathThreads / 32 + 2 and cute::elect_one_sync()) {
            ...
            for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
                ...
                if constexpr (kMajorB == cute::UMMA::Major::K)
                    tma_copy<BLOCK_K, LOAD_BLOCK_N, kSwizzleBMode, int8_t, kIsBatchedMM>(
                        &tensor_map_b, full_barriers_b[0], smem_b[0], k_idx, n_idx, kNumMulticast, batch_idx);
                if constexpr (kMajorB == cute::UMMA::Major::MN)
                    tma_copy<LOAD_BLOCK_N, BLOCK_K, kSwizzleBMode, int8_t, kIsBatchedMM>(
                ...
                for (uint32_t block_m_iter = scheduler.m_start; block_m_iter < scheduler.m_end; advance_pipeline(block_m_iter)) {
                    ...
                    if constexpr (kMajorA == cute::UMMA::Major::K)
                        tma_copy<BLOCK_K, LOAD_BLOCK_M, kSwizzleAMode, int8_t, kIsBatchedMM>(
                            &tensor_map_a, full_barriers[stage_idx], smem_a[stage_idx], k_idx, m_idx, kNumMulticast, batch_idx);
                    if constexpr (kMajorA == cute::UMMA::Major::MN)
                        tma_copy<LOAD_BLOCK_M, BLOCK_K, kSwizzleAMode, int8_t, kIsBatchedMM>(
        ...
        for (int block_k_iter = 0; block_k_iter < block_k; ++block_k_iter) {
            ...
            // Inner loop over M blocks
            for (uint32_t block_m_iter = scheduler.m_start; block_m_iter < scheduler.m_end; advance_pipeline(block_m_iter)) {
                // Accumulator in registers (int32 / S32) -- zero-initialized per M block
                ...
                for (uint32_t local_idx = 0; local_idx < kNumMWaves; ++ local_idx) {
                    ...
                    for (uint32_t k = 0; k < BLOCK_K / WGMMA::K; ++ k) {
                        ...
                        a_desc.reg32_[0] = advance_gmma_desc_lo<kMajorA, BLOCK_M, BLOCK_ATOM_K, kSwizzleAMode, int8_t>(
                            a_desc_base_lo, local_idx * WAVE_BLOCK_M, (k * WGMMA::K) % BLOCK_ATOM_K, atom_k_idx * BLOCK_M * BLOCK_ATOM_K);
                        b_desc.reg32_[0] = advance_gmma_desc_lo<kMajorB, BLOCK_N, BLOCK_ATOM_K, kSwizzleBMode, int8_t>(
                ...
                // Convert int32 accumulators to float, then apply SFA * SFB.
                #pragma unroll
                for (uint32_t local_idx = 0; local_idx < kNumMWaves; ++ local_idx) {
                    auto m_offset = local_idx * WAVE_BLOCK_M;
                    auto shifted_accum = accum + WGMMA::kNumAccum * local_idx;
                    ...
                    for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                        // Apply scale: final = scale_a * scale_b * float(raw_accum)
                ...
                // CD groups are folded into the M (outer) dimension of the tensor map, so the
                // store's N coordinate is group-LOCAL (scheduler.n_idx is group-global, used only
                // for the A/B/SF loads whose tensor maps stack groups along N).
                ...
                if (threadIdx.x < BLOCK_N / TMA_D_BLOCK_N) {
                    ...
                    auto smem_ptr = smem_cd[tma_stage_idx] + in_block_n_offset;
