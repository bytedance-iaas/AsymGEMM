...
sm100_fp4_asym_gemm_1d1d_impl(uint32_t* offsets, uint32_t* experts,
    ...
    // return immediately. The init phase above already ran Allocator().allocate()
    // in warp 2 unconditionally, so the TMEM allocation must be released before
    // returning, or the driver reports `cudaErrorTensorMemoryLeak`. Mirror the
    // one-warp-frees pattern used on the normal exit path (warp 2 owned the
    // allocation). cluster_sync() has already completed and no TMA / TMEM
    // barriers have been armed, so no barrier is left in an un-arrived state.
    if constexpr (kGemmType == GemmType::MGroupedMasked) {
        if (scheduler.m_end == 0) {
            if (warp_idx == 2) {
