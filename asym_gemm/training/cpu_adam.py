...
def _pin_if_requested(tensor: torch.Tensor, *, pin_memory: bool) -> tuple[torch.Tensor, str | None]:
    ...
    if exact_pinned_enabled() and tensor.is_contiguous() and register_inplace(tensor) is None:
        # capacity fix 2026-07-25 (38-merge): page-lock the exact-size tensor in
        # place (exact_pinned.py) instead of pin_memory()'s pow2-bucketed copy;
        # ledger accounting stays truthful — same reserve, registered in place.
        pinned_ledger.register_tensor(tensor, "adam")
        return tensor, None
    try:
