"""GLM-4.5-Air (glm4_moe) family wrapper (model_integration.md, #4 of 6).

REPLICATED code path (campaign rule): standalone family-facing layer; engine =
shared `AsymQwen3Experts` (tf-5.6 `Glm4MoeNaiveMoe` carries the identical
packed layout `[E, 2I, H]/[E, H, I]`, silu, standard forward contract).

Family deltas vs the Mixtral/Phimoe/Hunyuan clones:
- the gate (`Glm4MoeTopkRouter`) returns raw fp32 LOGITS; DeepSeek-V3-style
...
- first-k-dense / "dense" `mlp_layer_types` layers are plain `Glm4MoeMLP`
...
def _name_matches(obj: object) -> bool:
    ...
    return "glm4moe" in lowered and "lite" not in lowered
...
def _resolve_glm45_expert_act_fn(module: nn.Module):
...
def is_glm45_experts(module: nn.Module) -> bool:
    ...
    for attr in ("num_experts", "hidden_dim", "intermediate_dim"):
        if not isinstance(getattr(module, attr, None), int):
            return False
    if not callable(_resolve_glm45_expert_act_fn(module)):
        return False
    try:
...
def is_glm45_moe_block(module: nn.Module) -> bool:
    if getattr(module, "_is_asym_glm45_moe_block", False):
        return False
    if not _name_matches(module):
        return False
    ...
    if not isinstance(gate, nn.Module) or not is_glm45_experts(experts):
        return False
...
class AsymGlm45MoeBlock(nn.Module):
    """GLM-4.5-Air MoE block wrapper: frozen gate + replicated group top-k + shared experts."""

    _is_asym_glm45_moe_block = True
    ...
    def __init__(
        ...
        *,
        ...
        if router_mode != "whole":
            raise ValueError(f"AsymGlm45MoeBlock only implements router_mode='whole', got {router_mode!r}")
        if strict and not is_glm45_moe_block(source):
            source_file = inspect.getsourcefile(type(source)) or "unknown"
            raise TypeError(
                "source does not look like a GLM-4.5-Air MoE block with gate/experts/shared_experts: "
    ...
    def _route_tokens_to_experts(self, router_logits: torch.Tensor):
        # Verbatim replication of Glm4MoeMoE.route_tokens_to_experts.
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if hidden_states.dim() != 3:
            raise ValueError(f"AsymGlm45MoeBlock expects [batch, seq, hidden], got {tuple(hidden_states.shape)}")
