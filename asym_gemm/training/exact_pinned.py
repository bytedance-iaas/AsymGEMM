...
def register_inplace(tensor: torch.Tensor) -> str | None:
    ...
    # 39-merge guard (2026-08-01, mrg38on incident): transformers 5.x loads weights as
    # safetensors MMAP slices — with clone=False call sites the tensor ALIASES the
    # file-backed slab. Registering that (a) pins the whole multi-GB file mapping, not
    # the tensor, and (b) async DMA from file-backed registered pages throws
    # cudaErrorInvalidValue on this stack (GB200 / torch 2.12 / CUDA 13). pin_memory()
    # copies out of the mmap, so falling through IS the stock-correct behavior.
    if getattr(storage, "filename", None) is not None:
        return "mmap_backed"
    nbytes = storage.nbytes()
    if _env_flag("ASYM_EXACT_PINNED_DEBUG"):
        import traceback

        caller = "".join(traceback.format_stack(limit=4)[:-1]).splitlines()
        site = next((ln.strip() for ln in reversed(caller) if "asym_gemm" in ln and "exact_pinned" not in ln), "?")
        print(
            f"[exact-pinned-debug] register req: tensor={tuple(tensor.shape)} {tensor.dtype} "
            f"tensor_bytes={tensor.numel()*tensor.element_size()} storage_bytes={nbytes} "
            f"ptr%4096={tensor.data_ptr()%4096} storage_off={tensor.data_ptr()-storage.data_ptr()} site={site}",
            flush=True,
        )
    ...
    if _env_flag("ASYM_EXACT_PINNED_DEBUG"):
        try:
            probe = tensor.view(-1)[: 2048 // max(1, tensor.element_size())].to("cuda", non_blocking=False)
            del probe
            verdict = "PROBE=OK"
        except Exception as exc:  # pragma: no cover - diagnostic only
            verdict = f"PROBE=FAIL:{type(exc).__name__}:{str(exc)[:60]}"
        print(
            f"[exact-pinned-debug] reg#{_REGISTERED_COUNT} total={_REGISTERED_BYTES/(1<<30):.1f}GiB {verdict}",
            flush=True,
        )
    return None
