...
# T3 recipe: fix2+clone infra + ohbm56 (1 root; GPU ~97% ceiling at 416K)
...
set +e
guard T2B416
echo "=== [1] T2B(fix2+clone) @416K capacity attempt (w1+m1) $(date -u +%H:%M:%S)"
env $XPC WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
  bash scripts/lf/tp_probe.sh mixtral-8x22b m6mxg1t2b416 "asym_cpuadamwds|T2B|ligerloss1" 416000 1
T2B416=$?
echo "R15_T2B416_EXIT=$T2B416"
guard T3416
echo "=== [2] T3(fix2+clone,ohbm56,blockexp,cap,skip) @416K (w1+m1) $(date -u +%H:%M:%S)"
env $XPC \
...
WARMUP_STEPS=1 MAX_STEPS=1 MAX_SAMPLES=512 \
  bash scripts/lf/tp_probe.sh mixtral-8x22b m6mxg1t3416 "asym_cpuadamwds|recomp-off-full-fg-ker000-ceil0000-ohbm56|ligerloss1" 416000 1
T3416=$?
echo "R15_T3416_EXIT=$T3416"
if [ $T3416 -eq 0 ] && [ $T2B416 -ne 0 ]; then echo "PROBE15_WINNER=416000"; exit 0; fi
echo "PROBE15_STATE=t2b=$T2B416,t3=$T3416"
exit 3
